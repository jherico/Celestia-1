 ...
 *  is const.
 */
Material::Pointer Model::getMaterial(unsigned int index) const {
 ...
 */
unsigned int Model::addMaterial(const Material::Pointer& m) {
...
unsigned int Model::getPrimitiveCount() const {
    unsigned int count = 0;
    for (const auto& mesh : meshes) {
...
Mesh::Pointer Model::getMesh(unsigned int index) const {
...
unsigned int Model::addMesh(const Mesh::Pointer& m) {
    meshes.push_back(m);
    return meshes.size();
}
...
bool Model::OpacityComparator::operator()(const Mesh& a, const Mesh& b) const {
...
static unsigned int getMeshMaterialIndex(const Mesh& mesh) {
