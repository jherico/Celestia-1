...
SampledOrientation::SampledOrientation() : lastSample(0), interpolation(Linear) {
}

SampledOrientation::~SampledOrientation() {
}

void SampledOrientation::addSample(double t, const Eigen::Quaternionf& q) {
...
Eigen::Quaterniond SampledOrientation::spin(double tjd) const {
...
Quaternionf SampledOrientation::getOrientation(double tjd) const {
    ...
    if (samples.size() == 0) {
        ...
        if (n < 1 || n >= (int)samples.size() || tjd < samples[n - 1].t || tjd > samples[n].t) {
            OrientationSampleVector::const_iterator iter = lower_bound(samples.begin(), samples.end(), samp);
            if (iter == samples.end())
                n = samples.size();
            else
...
RotationModel::Pointer LoadSampledOrientation(const string& filename) {
    ifstream in(filename.c_str());
    if (!in.good())
        return NULL;

    auto sampOrientation = std::make_shared<SampledOrientation>();
    int nSamples = 0;
    while (in.good()) {
        double tjd;
        Quaternionf q;
