...
public:
    using Pointer = std::shared_ptr<SpiceRotation>;
    SpiceRotation(const std::string& frameName,
                  const std::string& baseFrameName,
                  double period,
                  double beginning,
                  double ending);
    SpiceRotation(const std::string& frameName, const std::string& baseFrameName, double period);
    ...
    Eigen::Quaterniond computeEquatorOrientation(double /* tdb */) const { return Eigen::Quaterniond::Identity(); }
