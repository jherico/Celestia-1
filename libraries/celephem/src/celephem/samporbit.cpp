...
template <typename T>
bool operator<(const SampleXYZV<T>& a, const SampleXYZV<T>& b) {
    return a.t < b.t;
}
...
template <typename T>
SampledOrbit<T>::SampledOrbit(TrajectoryInterpolation _interpolation) : interpolation(_interpolation) {
}

template <typename T>
SampledOrbit<T>::~SampledOrbit() {
}

template <typename T>
void SampledOrbit<T>::addSample(double t, double x, double y, double z) {
...
template <typename T>
double SampledOrbit<T>::getPeriod() const {
    return samples[samples.size() - 1].t - samples[0].t;
}

template <typename T>
bool SampledOrbit<T>::isPeriodic() const {
    return false;
}

template <typename T>
void SampledOrbit<T>::getValidRange(double& begin, double& end) const {
    begin = samples[0].t;
    end = samples[samples.size() - 1].t;
}

template <typename T>
double SampledOrbit<T>::getBoundingRadius() const {
    return boundingRadius;
}

static Vector3d cubicInterpolate(const Vector3d& p0, const Vector3d& v0, const Vector3d& p1, const Vector3d& v1, double t) {
    return p0 + (((2.0 * (p0 - p1) + v1 + v0) * (t * t * t)) + ((3.0 * (p1 - p0) - 2.0 * v0 - v1) * (t * t)) + (v0 * t));
}

static Vector3d cubicInterpolateVelocity(const Vector3d& p0,
                                         const Vector3d& v0,
                                         const Vector3d& p1,
                                         const Vector3d& v1,
                                         double t) {
    return ((2.0 * (p0 - p1) + v1 + v0) * (3.0 * t * t)) + ((3.0 * (p1 - p0) - 2.0 * v0 - v1) * (2.0 * t)) + v0;
}

template <typename T>
Vector3d SampledOrbit<T>::computePosition(double jd) const {
    Vector3d pos;
    if (samples.size() == 0) {
        pos = Vector3d::Zero();
        ...
        if (n == 0) {
            ...
            if (interpolation == TrajectoryInterpolationLinear) {
                ...
                pos = Vector3d(Mathd::lerp(t, (double)s0.x, (double)s1.x), Mathd::lerp(t, (double)s0.y, (double)s1.y),
                ...
                Vector3d v10((double)s1.x - (double)s0.x, (double)s1.y - (double)s0.y, (double)s1.z - (double)s0.z);
                Vector3d v21((double)s2.x - (double)s1.x, (double)s2.y - (double)s1.y, (double)s2.z - (double)s1.z);
                Vector3d v32((double)s3.x - (double)s2.x, (double)s3.y - (double)s2.y, (double)s3.z - (double)s2.z);
...
template <typename T>
Vector3d SampledOrbit<T>::computeVelocity(double jd) const {
    Vector3d vel;
    if (samples.size() < 2) {
        vel = Vector3d::Zero();
    } else {
...
template <typename T>
void SampledOrbit<T>::sample(double /* startTime */, double /* endTime */, const OrbitSampleProc& proc) const {
    for (uint32_t i = 0; i < samples.size(); i++) {
        ...
        proc(samples[i].t, Vector3d(p.x(), p.z(), -p.y()), Vector3d(v.x(), v.z(), -v.y()));
    }
}
...
template <typename T>
SampledOrbitXYZV<T>::SampledOrbitXYZV(TrajectoryInterpolation _interpolation) : interpolation(_interpolation) {
}

template <typename T>
SampledOrbitXYZV<T>::~SampledOrbitXYZV() {
}
...
template <typename T>
void SampledOrbitXYZV<T>::addSample(double t, const Vector3d& position, const Vector3d& velocity) {
...
template <typename T>
double SampledOrbitXYZV<T>::getPeriod() const {
    if (samples.empty())
        return 0.0;
    else
        return samples[samples.size() - 1].t - samples[0].t;
...
template <typename T>
bool SampledOrbitXYZV<T>::isPeriodic() const {
    return false;
}

template <typename T>
void SampledOrbitXYZV<T>::getValidRange(double& begin, double& end) const {
    begin = samples[0].t;
    end = samples[samples.size() - 1].t;
}

template <typename T>
double SampledOrbitXYZV<T>::getBoundingRadius() const {
    return boundingRadius;
}

template <typename T>
Vector3d SampledOrbitXYZV<T>::computePosition(double jd) const {
    Vector3d pos;
    if (samples.size() == 0) {
        pos = Vector3d::Zero();
    } else if (samples.size() == 1) {
        ...
        if (n < 1 || n >= (int)samples.size() || jd < samples[n - 1].t || jd > samples[n].t) {
            typename vector<SampleXYZV<T>>::const_iterator iter = lower_bound(samples.begin(), samples.end(), samp);
            if (iter == samples.end())
                n = samples.size();
            else
                n = iter - samples.begin();
...
// for position.
template <typename T>
Vector3d SampledOrbitXYZV<T>::computeVelocity(double jd) const {
    ...
    if (samples.size() >= 2) {
        ...
        if (n > 0 && n < (int)samples.size()) {
            ...
            if (interpolation == TrajectoryInterpolationLinear) {
                double h = s1.t - s0.t;
                vel = Vector3d(s1.position.x() - s0.position.x(), s1.position.y() - s0.position.y(),
                               s1.position.z() - s0.position.z()) *
                      (1.0 / h) * astro::daysToSecs(1.0);
            } else if (interpolation == TrajectoryInterpolationCubic) {
                double h = s1.t - s0.t;
                double ih = 1.0 / h;
                double t = (jd - s0.t) * ih;
...
template <typename T>
void SampledOrbitXYZV<T>::sample(double /* startTime */, double /* endTime */, const OrbitSampleProc& proc) const {
    for (typename vector<SampleXYZV<T>>::const_iterator iter = samples.begin(); iter != samples.end(); iter++) {
        proc(iter->t, Vector3d(iter->position.x(), iter->position.z(), -iter->position.y()),
...
template <typename T>
std::shared_ptr<SampledOrbit<T>> LoadSampledOrbit(const string& filename, TrajectoryInterpolation interpolation, T) {
    ...
    using Type = SampledOrbit<T>;
    auto orbit = std::make_shared<Type>(interpolation);
...
template <typename T>
std::shared_ptr<SampledOrbitXYZV<T>> LoadSampledOrbitXYZV(const string& filename, TrajectoryInterpolation interpolation, T) {
    ifstream in(filename.c_str());
    if (!in.good())
        return NULL;
    ...
    auto orbit = std::make_shared<SampledOrbitXYZV<T>>(interpolation);
 ...
 */
Orbit::Pointer LoadSampledTrajectorySinglePrec(const string& filename, TrajectoryInterpolation interpolation) {
    return LoadSampledOrbit(filename, interpolation, 0.0f);
}

/*! Load a trajectory file containing double precision positions.
 */
Orbit::Pointer LoadSampledTrajectoryDoublePrec(const string& filename, TrajectoryInterpolation interpolation) {
    return LoadSampledOrbit(filename, interpolation, 0.0);
}

/*! Load a trajectory file with single precision positions and velocities.
 */
Orbit::Pointer LoadXYZVTrajectorySinglePrec(const string& filename, TrajectoryInterpolation interpolation) {
 ...
 */
Orbit::Pointer LoadXYZVTrajectoryDoublePrec(const string& filename, TrajectoryInterpolation interpolation) {
