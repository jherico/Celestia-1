...
/** Sample the orbit over the time range [ startTime, endTime ] using the
  ...
  */
void Orbit::sample(double startTime, double endTime, const OrbitSampleProc& proc) const {
    double span = 0.0;
    if (isPeriodic()) {
        span = getPeriod();
    } else {
...
/** Adaptively sample the orbit over the range [ startTime, endTime ].
  */
void Orbit::adaptiveSample(double startTime,
                           double endTime,
    ...
    while (t < endTime) {
        ...
        Vector3d pInterp = cubicInterpolate(lastP, lastV * dt, p1, v1 * dt, 0.5);
        nTests++;
...
EllipticalOrbit::EllipticalOrbit(double _pericenterDistance,
    ...
    eccentricity(_eccentricity), inclination(_inclination), ascendingNode(_ascendingNode), argOfPeriapsis(_argOfPeriapsis),
    meanAnomalyAtEpoch(_meanAnomalyAtEpoch), period(_period), epoch(_epoch) {
...
struct SolveKeplerFunc1 : public unary_function<double, double> {
    ...
    double operator()(double x) const { return M + ecc * sin(x); }
};
...
struct SolveKeplerFunc2 : public unary_function<double, double> {
    ...
    double operator()(double x) const { return x + (M + ecc * sin(x) - x) / (1 - ecc * cos(x)); }
};
...
CachingOrbit::CachingOrbit() : lastTime(-1.0e30), positionCacheValid(false), velocityCacheValid(false) {
}
...
static EllipticalOrbit::Pointer StateVectorToOrbit(const Vector3d& position, const Vector3d& v, double mass, double t) {
    Vector3d R = position;
    ...
    return std::make_shared<EllipticalOrbit>(a * (1 - e), e, i, Om, om, M, T, t);
}

MixedOrbit::MixedOrbit(const Orbit::Pointer& orbit, double t0, double t1, double mass) :
    primary(orbit), begin(t0), end(t1), boundingRadius(0.0) {
    assert(t1 > t0);
    assert(orbit != NULL);
...
void MixedOrbit::sample(double startTime, double endTime, const OrbitSampleProc& proc) const {
    Orbit::Pointer o;
    if (startTime < begin)
        o = beforeApprox;
...
FixedOrbit::FixedOrbit(const Vector3d& pos) : position(pos) {
}

FixedOrbit::~FixedOrbit() {
}

Vector3d FixedOrbit::positionAtTime(double /*tjd*/) const {
    return position;
}

bool FixedOrbit::isPeriodic() const {
    return false;
}
...
double FixedOrbit::getBoundingRadius() const {
    return position.norm() * 1.1;
}

void FixedOrbit::sample(double /* startTime */, double /* endTime */, const OrbitSampleProc&) const {
...
SynchronousOrbit::SynchronousOrbit(const Body& _body, const Vector3d& _position) : body(_body), position(_position) {
}
...
void SynchronousOrbit::sample(double /* startTime */, double /* endTime */, const OrbitSampleProc&) const {
