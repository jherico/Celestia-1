...
SpiceOrbit::SpiceOrbit(const std::string& _targetBodyName,
    ...
    originName(_originName), period(_period), boundingRadius(_boundingRadius), spiceErr(false), validIntervalBegin(_beginning),
    validIntervalEnd(_ending), useDefaultTimeInterval(false) {
}
...
SpiceOrbit::SpiceOrbit(const std::string& _targetBodyName,
                       ...
                       double _boundingRadius) :
    targetBodyName(_targetBodyName),
    originName(_originName), period(_period), boundingRadius(_boundingRadius), spiceErr(false), validIntervalBegin(0.0),
    validIntervalEnd(0.0), useDefaultTimeInterval(true) {
}

SpiceOrbit::~SpiceOrbit() {
}

bool SpiceOrbit::isPeriodic() const {
    return period != 0.0;
};

double SpiceOrbit::getPeriod() const {
    if (isPeriodic()) {
        return period;
    } else {
...
bool SpiceOrbit::init(const string& path, const list<string>& requiredKernels) {
    // Load required kernel files
    for (auto kernel : requiredKernels) {
    ...
    if (useDefaultTimeInterval) {
        ...
        if (targetID != 0 && !wnincd_c(beginningSecondsJ2000, endingSecondsJ2000, &targetCoverage)) {
    ...
    double position[3];
    double lt = 0.0;
    spkgps_c(targetID, beginning, "eclipj2000", originID, position, &lt);
    if (failed_c()) {
...
Vector3d SpiceOrbit::computePosition(double jd) const {
    ...
    if (spiceErr) {
        ...
        spkgps_c(targetID, t, "eclipj2000", originID, position, &lt);
...
Vector3d SpiceOrbit::computeVelocity(double jd) const {
    if (jd < validIntervalBegin)
        jd = validIntervalBegin;
    else if (jd > validIntervalEnd)
        jd = validIntervalEnd;
    ...
    if (spiceErr) {
        ...
        spkgeo_c(targetID, t, "eclipj2000", originID, state, &lt);
