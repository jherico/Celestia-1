...
SpiceRotation::SpiceRotation(const std::string& frameName,
    ...
    m_baseFrameName(baseFrameName), m_period(period), m_spiceErr(false), m_validIntervalBegin(beginning),
    m_validIntervalEnd(ending), m_useDefaultTimeInterval(false) {
}
 ...
 *  models.
 */
SpiceRotation::SpiceRotation(const std::string& frameName, const std::string& baseFrameName, double period) :
    m_frameName(frameName), m_baseFrameName(baseFrameName), m_period(period), m_spiceErr(false),
    m_validIntervalBegin(-numeric_limits<double>::infinity()), m_validIntervalEnd(numeric_limits<double>::infinity()),
...
bool SpiceRotation::isPeriodic() const {
    return m_period != 0.0;
};

double SpiceRotation::getPeriod() const {
    if (isPeriodic()) {
        return m_period;
    } else {
...
bool SpiceRotation::init(const string& path, const list<string>& requiredKernels) {
...
Quaterniond SpiceRotation::computeSpin(double jd) const {
    ...
    if (m_spiceErr) {
        ...
        double matrixData[9] = { xform[0][0], xform[0][1], xform[0][2], xform[1][0], xform[1][1],
                                 xform[1][2], xform[2][0], xform[2][1], xform[2][2] };
