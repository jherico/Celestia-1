...
static const Quaterniond EQUATORIAL_TO_GALACTIC_ROTATION = ZRotation(degToRad(GALACTIC_NODE)) *
...
static const char* MonthAbbrList[12] = { "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
#endif
...
static const UnitDefinition timeUnits[] = {
    { "s", 1.0 / SECONDS_PER_DAY }, { "min", 1.0 / MINUTES_PER_DAY }, { "h", 1.0 / HOURS_PER_DAY }, { "d", 1.0 },
    { "y", DAYS_PER_YEAR },
};
...
float astro::sphereIlluminationFraction(Point3d, Point3d) {
    return 1.0f;
}
...
// coordinates.
Eigen::Vector3f astro::equatorialToCelestialCart(float ra, float dec, float distance) {
...
// Convert equatorial coordinates to Cartesian celestial (or ecliptical)
// coordinates.
Eigen::Vector3d astro::equatorialToCelestialCart(double ra, double dec, double distance) {
    double theta = ra / 24.0 * PI * 2 + PI;
    double phi = (dec / 90.0 - 1.0) * PI / 2;
    double x = cos(theta) * sin(phi) * distance;
    double y = cos(phi) * distance;
...
/** Convert spherical coordinates in the J2000 equatorial frame to cartesian
  ...
  */
Eigen::Vector3f astro::equatorialToEclipticCartesian(float ra, float dec, float distance) {
    double theta = ra / 24.0 * PI * 2 + PI;
    double phi = (dec / 90.0 - 1.0) * PI / 2;
    double x = cos(theta) * sin(phi) * distance;
    double y = cos(phi) * distance;
...
const char* astro::Date::toCStr(Format format) const {
    ...
    switch (format) {
        case Locale:
        case TZName:
            snprintf(date, sizeof(date), "%04d %s %02d %02d:%02d:%02d %s", year, _(MonthAbbrList[month - 1]), day, hour, minute,
                     (int)seconds, tzname.c_str());
            break;
        case UTCOffset: {
            int sign = utc_offset < 0 ? -1 : 1;
            int h_offset = sign * utc_offset / 3600;
            int m_offset = (sign * utc_offset - h_offset * 3600) / 60;
            snprintf(date, sizeof(date), "%04d %s %02d %02d:%02d:%02d %c%02d%02d", year, _(MonthAbbrList[month - 1]), day, hour,
                     minute, (int)seconds, (sign == 1 ? '+' : '-'), h_offset, m_offset);
        } break;
    }
#endif
...
astro::Date::operator double() const {
    ...
    return (floor(365.25 * y) + floor(30.6001 * (m + 1)) + B + 1720996.5 + day + hour / HOURS_PER_DAY +
            minute / MINUTES_PER_DAY + seconds / SECONDS_PER_DAY);
}
...
bool astro::parseDate(const string& s, astro::Date& date) {
    ...
    if (sscanf(s.c_str(), " %d %u %u %u:%u:%lf ", &year, &month, &day, &hour, &minute, &second) == 6 ||
        sscanf(s.c_str(), " %d %u %u %u:%u ", &year, &month, &day, &hour, &minute) == 5 ||
...
astro::Date astro::Date::systemDate() {
    time_t t = time(NULL);
...
astro::Date astro::TAItoUTC(double tai) {
    uint32_t nRecords = sizeof(LeapSeconds) / sizeof(LeapSeconds[0]);
    double dAT = LeapSeconds[0].seconds;
    /*double dD = 0.0;  Unused*/
    int extraSecs = 0;

    for (uint32_t i = nRecords - 1; i > 0; i--) {
...
double astro::UTCtoTAI(const astro::Date& utc) {
    uint32_t nRecords = sizeof(LeapSeconds) / sizeof(LeapSeconds[0]);
    double dAT = LeapSeconds[0].seconds;
        ...
        if (localt != NULL) {
...
#else
            {
                ...
                if (daydiff == 0)
                    ...
                    hourdiff = 24;
                d.utc_offset = (hourdiff + d.hour - utcDate.hour) * 3600 + (d.minute - utcDate.minute) * 60;
            }
...
double astro::UTCtoTDB(const astro::Date& utc) {
...
// seconds.
double astro::JDUTCtoTAI(double utc) {
    uint32_t nRecords = sizeof(LeapSeconds) / sizeof(LeapSeconds[0]);
    double dAT = LeapSeconds[0].seconds;
...
double astro::TAItoJDUTC(double tai) {
    uint32_t nRecords = sizeof(LeapSeconds) / sizeof(LeapSeconds[0]);
    double dAT = LeapSeconds[0].seconds;
...
bool astro::getLengthScale(string unitName, double& scale) {
    uint32_t nUnits = sizeof(lengthUnits) / sizeof(lengthUnits[0]);
...
bool astro::getTimeScale(string unitName, double& scale) {
    uint32_t nUnits = sizeof(timeUnits) / sizeof(timeUnits[0]);
    bool foundMatch = false;
    for (uint32_t i = 0; i < nUnits; i++) {
...
bool astro::getAngleScale(string unitName, double& scale) {
    uint32_t nUnits = sizeof(angleUnits) / sizeof(angleUnits[0]);
    bool foundMatch = false;
    for (uint32_t i = 0; i < nUnits; i++) {
