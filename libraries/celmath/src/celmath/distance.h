...
template <class T>
T distance(const Point3<T>& p, const Sphere<T>& s) {
    return abs(s.center.distanceTo(p) - s.radius);
}

template <class T>
T distance(const Point3<T>& p, const Ellipsoid<T>& e) {
    return 0.0f;
}

template <class T>
T distance(const Point3<T>& p, const Ray3<T>& r) {
...
template <class T>
T distance(const Eigen::Matrix<T, 3, 1>& p, const Ray3<T>& r) {
...
template <class T>
T distanceToSegment(const Point3<T>& p, const Point3<T>& origin, const Vector3<T>& direction) {
