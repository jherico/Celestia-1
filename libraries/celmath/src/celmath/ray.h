...
public:
    ...
    Ray3<T> transform(const Eigen::Matrix<T, 3, 3>& m) const { return Ray3<T>(m * origin, m * direction); }
...
// Compatibility
template <class T>
Ray3<T>::Ray3(const Point3<T>& _origin, const Vector3<T>& _direction) :
    origin(_origin.x, _origin.y, _origin.z), direction(_direction.x, _direction.y, _direction.z) {
}

template <class T>
Eigen::Matrix<T, 3, 1> Ray3<T>::point(T t) const {
