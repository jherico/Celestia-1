...
public:
    ...
    static Quat<T> vecToVecRotation(const Vector3<T>& v0, const Vector3<T>& v1);
...
template <class T>
Quat<T>::Quat(const Quat<T>& q) : w(q.w), x(q.x), y(q.y), z(q.z) {
}

template <class T>
Quat<T>::Quat(T re) : w(re), x(0), y(0), z(0) {
}

// Create a 'pure' quaternion
template <class T>
Quat<T>::Quat(const Vector3<T>& im) : w(0), x(im.x), y(im.y), z(im.z) {
}

template <class T>
Quat<T>::Quat(T re, const Vector3<T>& im) : w(re), x(im.x), y(im.y), z(im.z) {
}

template <class T>
Quat<T>::Quat(T _w, T _x, T _y, T _z) : w(_w), x(_x), y(_y), z(_z) {
}
...
template <class T>
Quat<T>& Quat<T>::operator+=(Quat<T> a) {
    x += a.x;
    y += a.y;
    z += a.z;
    w += a.w;
...
template <class T>
Quat<T>& Quat<T>::operator*=(Quat<T> q) {
    *this = Quat<T>(w * q.w - x * q.x - y * q.y - z * q.z, w * q.x + x * q.w + y * q.z - z * q.y,
...
template <class T>
Quat<T> operator*(Quat<T> a, Quat<T> b) {
    return Quat<T>(a.w * b.w - a.x * b.x - a.y * b.y - a.z * b.z, a.w * b.x + a.x * b.w + a.y * b.z - a.z * b.y,
                   a.w * b.y + a.y * b.w + a.z * b.x - a.x * b.z, a.w * b.z + a.z * b.w + a.x * b.y - a.y * b.x);
}

template <class T>
Quat<T> operator*(T s, Quat<T> q) {
    return Quat<T>(s * q.w, s * q.x, s * q.y, s * q.z);
}

template <class T>
Quat<T> operator*(Quat<T> q, T s) {
    return Quat<T>(s * q.w, s * q.x, s * q.y, s * q.z);
}

// equivalent to multiplying by the quaternion (0, v)
template <class T>
Quat<T> operator*(Vector3<T> v, Quat<T> q) {
    return Quat<T>(-v.x * q.x - v.y * q.y - v.z * q.z, v.x * q.w + v.y * q.z - v.z * q.y, v.y * q.w + v.z * q.x - v.x * q.z,
...
template <class T>
bool operator==(Quat<T> a, Quat<T> b) {
...
template <class T>
Quat<T> pow(Quat<T> q, T s) {
    return exp(s * log(q));
}

template <class T>
Quat<T> pow(Quat<T> q, Quat<T> p) {
    return exp(p * log(q));
}
...
Quat<T> sin(Quat<T> q) {
    if (q.isReal()) {
        ...
        return Quat<T>((T)0.5 * e0 * s, c0 * q.x, c0 * q.y, c0 * q.z) + Quat<T>((T)0.5 * e1 * s, c1 * q.x, c1 * q.y, c1 * q.z);
    }
}
...
Quat<T> cos(Quat<T> q) {
    if (q.isReal()) {
        ...
        return Quat<T>((T)0.5 * e0 * c, c0 * q.x, c0 * q.y, c0 * q.z) + Quat<T>((T)0.5 * e1 * c, c1 * q.x, c1 * q.y, c1 * q.z);
    }
}
...
template <class T>
bool Quat<T>::isReal() const {
    return (x == 0 && y == 0 && z == 0);
}

template <class T>
bool Quat<T>::isPure() const {
    return w == 0;
}
...
template <class T>
void Quat<T>::setAxisAngle(Vector3<T> axis, T angle) {
    T s, c;
...
template <class T>
Matrix4<T> Quat<T>::toMatrix4() const {
    T wx = w * x * 2;
    T wy = w * y * 2;
    T wz = w * z * 2;
    T xx = x * x * 2;
    ...
    return Matrix4<T>(Vector4<T>(1 - yy - zz, xy - wz, xz + wy, 0), Vector4<T>(xy + wz, 1 - xx - zz, yz - wx, 0),
                      Vector4<T>(xz - wy, yz + wx, 1 - xx - yy, 0), Vector4<T>(0, 0, 0, 1));
}

// Convert this (assumed to be normalized) quaternion to a rotation matrix
template <class T>
Matrix3<T> Quat<T>::toMatrix3() const {
    T wx = w * x * 2;
    T wy = w * y * 2;
    T wz = w * z * 2;
    T xx = x * x * 2;
    ...
    return Matrix3<T>(Vector3<T>(1 - yy - zz, xy - wz, xz + wy), Vector3<T>(xy + wz, 1 - xx - zz, yz - wx),
                      Vector3<T>(xz - wy, yz + wx, 1 - xx - yy));
}

template <class T>
T dot(Quat<T> a, Quat<T> b) {
    return a.x * b.x + a.y * b.y + a.z * b.z + a.w * b.w;
}
 ...
 */
template <class T>
Quat<T> Quat<T>::slerp(const Quat<T>& q0, const Quat<T>& q1, T t) {
 ...
 */
template <class T>
Quat<T> Quat<T>::vecToVecRotation(const Vector3<T>& v0, const Vector3<T>& v1) {
 ...
 */
template <class T>
Quat<T> Quat<T>::matrixToQuaternion(const Matrix3<T>& m) {
    Quat<T> q;
    T trace = m[0][0] + m[1][1] + m[2][2];
    T root;
 ...
 */
template <class T>
void Quat<T>::rotate(Vector3<T> axis, T angle) {
    Quat q;
...
template <class T>
Quat<T> Quat<T>::xrotation(T angle) {
    T s, c;
    Math<T>::sincos(angle * (T)0.5, s, c);
...
template <class T>
Quat<T> Quat<T>::yrotation(T angle) {
    T s, c;
    Math<T>::sincos(angle * (T)0.5, s, c);
...
template <class T>
Quat<T> Quat<T>::zrotation(T angle) {
    T s, c;
    Math<T>::sincos(angle * (T)0.5, s, c);
 ...
 */
template <class T>
Quat<T> Quat<T>::lookAt(const Point3<T>& from, const Point3<T>& to, const Vector3<T>& up) {
    Vector3<T> n = to - from;
    n.normalize();
    Vector3<T> v = n ^ up;
    v.normalize();
