...
public:
     ...
     *  at the origin.
     */
    Ellipsoid() : center(0, 0, 0), axes(1, 1, 1) {}

    /*! Created an ellipsoid with the specified semiaxes, centered
     *  at the origin.
     */
    Ellipsoid(const Eigen::Matrix<T, 3, 1>& _axes) : center(0, 0, 0), axes(_axes) {}

    /*! Create an ellipsoid with the specified center and semiaxes.
     */
    Ellipsoid(const Eigen::Matrix<T, 3, 1>& _center, const Eigen::Matrix<T, 3, 1>& _axes) : center(_center), axes(_axes) {}
     ...
     *  at the origin.
     */
    Ellipsoid(const Vector3<T>& _axes) : center(0, 0, 0), axes(_axes.x, _axes.y, _axes.z) {}

    /*! Create an ellipsoid with the specified center and semiaxes.
     */
    Ellipsoid(const Point3<T>& _center, const Vector3<T>& _axes) :
        center(_center.x, _center.y, _center.z), axes(_axes.x, _axes.y, _axes.z) {}
