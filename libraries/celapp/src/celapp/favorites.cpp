...
bool ReadFavoritesList(istream& in, FavoritesList& favorites) {
    ...
    while (tokenizer.nextToken() != Tokenizer::TokenEnd) {
        ...
        auto favParamsValue = parser.readValue();
        if (!favParamsValue || favParamsValue->getType() != Value::HashType) {
            DPRINTF(0, "Error parsing favorites entry %s\n", fav.name.c_str());
            return false;
        }

        auto favParams = favParamsValue->getHash();
...
#ifdef CELVEC
        fav.orientation.setAxisAngle(Vec3f((float)axis.x, (float)axis.y, (float)axis.z), (float)angle);
#endif
...
void WriteFavoritesList(const FavoritesList& favorites, ostream& out) {
    for (const auto& fav : favorites) {
        ...
        Vector3d offset((double)(fav.position.x - (BigFix)baseUly.x()), (double)(fav.position.y - (BigFix)baseUly.y()),
