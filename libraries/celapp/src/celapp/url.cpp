...
const string getEncodedObjectName(const Selection& sel, const CelestiaCorePtr& appCore);

CelestiaState::CelestiaState() {
}

void CelestiaState::captureState(const CelestiaCorePtr& appCore) {
...
Url::Url(const std::string& str, CelestiaCore* core) :
    urlStr(str), appCore(core), pauseState(false), timeSource(UseUrlTime), version(2) {
...
Url::Url(const CelestiaCorePtr& core, UrlType type) {
    appCore = core;
    version = 2;
    ...
    switch (type) {
        case Absolute:
            snprintf(date_str, sizeof(date_str), "%04d-%02d-%02dT%02d:%02d:%08.5f", date.year, date.month, date.day, date.hour,
...
Url::Url(const CelestiaState& appState, unsigned int _version, TimeSource _timeSource) {
    ...
    u << "&ow=" << orientation.w() << "&ox=" << orientation.x() << "&oy=" << orientation.y() << "&oz=" << orientation.z();
...
void Url::initVersion2(std::map<std::string, std::string>& params, const std::string& timeString) {
    if (type != Settings) {
        if (params["dist"] != "")
            type = Relative;
        else
    ...
    switch (type) {
        case Absolute:
            date = astro::Date(0.0);
            sscanf(timeString.c_str(), "%d-%d-%dT%d:%d:%lf", &date.year, &date.month, &date.day, &date.hour, &date.minute,
                   &date.seconds);

            coord = UniversalCoord(BigFix(params["x"]), BigFix(params["y"]), BigFix(params["z"]));
...
void Url::initVersion3(std::map<std::string, std::string>& params, const std::string& timeString) {
...
void Url::evalName() {
    ...
    switch (type) {
        case Absolute:
            ...
            if (body1 != "")
                name += " " + std::string(_(getBodyShortName(body1).c_str()));
            if (body2 != "")
                name += " " + std::string(_(getBodyShortName(body2).c_str()));
            if (trackedStr != "")
                name += " -> " + std::string(_(getBodyShortName(trackedStr).c_str()));
            if (selectedStr != "")
                name += " [" + std::string(_(getBodyShortName(selectedStr).c_str())) + "]";
            break;
        case Relative:
            if (selectedStr != "")
                name = std::string(_(getBodyShortName(selectedStr).c_str())) + " ";
...
std::map<std::string, std::string> Url::parseUrlParams(const std::string& url) const {
    ...
    while (pos != std::string::npos) {
        ...
        if (startValue != std::string::npos) {
            startValue++;
            if (pos != std::string::npos)
                params[url.substr(startName, startValue - startName - 1)] =
                    decodeString(url.substr(startValue, pos - startValue));
            else
...
static std::string getBodyName(const UniversePtr& universe, const BodyPtr& body) {
    std::string name = body->getName();
    auto parentSystem = body->getSystem();
    BodyPtr parentBody;

    assert(parentSystem != NULL);
    ...
    if (version >= 3) {
        ...
        switch (type) {
            case Absolute:
                ...
                break;
            case Relative:
                sim->gotoSelectionLongLat(0, distance, (float)(longitude * PI / 180), (float)(latitude * PI / 180),
                                          Vector3f::UnitY());
                break;
            case Settings:
                break;
...
string Url::getEncodedObjectName(const Selection& selection) {
    return ::getEncodedObjectName(selection, appCore);
}

const string getEncodedObjectName(const Selection& selection, const CelestiaCorePtr& appCore) {
    ...
    switch (selection.getType()) {
        ...
        case Selection::Type_Location:
            ...
            {
                auto parentBody = selection.location()->getParentBody();
                if (parentBody)
