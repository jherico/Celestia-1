 ...
 *  new blocks.
 */
MemoryPool::MemoryPool(uint32_t alignment, uint32_t blockSize) :
 ...
 */
void*
MemoryPool::allocate(uint32_t size)
{
    if (size > m_blockSize)
        return NULL;
    ...
    // Advance
    uint32_t padSize = (size + m_alignment - 1) & ~(m_alignment - 1);
...
MemoryPool::freeAll()
{
    ...
    for (list<Block>::iterator iter = m_blockList.begin(); iter != m_blockList.end(); iter++)
    {
        uint32_t* p = reinterpret_cast<uint32_t*>(iter->m_memory);
        for (uint32_t i = 0; i < m_blockSize / sizeof(uint32_t); i++)
            p[i] = 0xdeaddead;
    }
#endif
    m_currentBlock = m_blockList.begin();
