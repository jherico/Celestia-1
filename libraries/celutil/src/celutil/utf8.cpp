...
uint32_t WGL4_Normalization_00[256] = {
...
uint32_t WGL4_Normalization_01[256] = {
...
uint32_t WGL4_Normalization_02[256] = {
...
uint32_t WGL4_Normalization_03[256] = {
...
uint32_t WGL4_Normalization_04[256] = {
...
uint32_t WGL4_Normalization_1e[256] = {
...
uint32_t WGL4_Normalization_21[256] = {
...
uint32_t* WGL4NormalizationTables[256] = {
...
{
    uint32_t c0 = (uint32_t) str[pos];
    size_t charlen = UTF8EncodedSizeFromFirstByte(c0);
    ...
    case 2:
        ch = ((c0 & 0x1f) << 6) | ((uint32_t) str[pos + 1] & 0x3f);
        return true;

    case 3:
        ch = ((c0 & 0x0f) << 12) |
            (((uint32_t) str[pos + 1] & 0x3f) << 6) |
            ((uint32_t)  str[pos + 2] & 0x3f);
        return true;

    case 4:
        ch = ((c0 & 0x07) << 18) |
            (((uint32_t) str[pos + 1] & 0x3f) << 12) |
            (((uint32_t) str[pos + 2] & 0x3f) << 6)  |
            ((uint32_t)  str[pos + 3] & 0x3f);
        return true;

    case 5:
        ch = ((c0 & 0x03) << 24) |
            (((uint32_t) str[pos + 1] & 0x3f) << 18) |
            (((uint32_t) str[pos + 2] & 0x3f) << 12) |
            (((uint32_t) str[pos + 3] & 0x3f) << 6)  |
            ((uint32_t)  str[pos + 4] & 0x3f);
        return true;

    case 6:
        ch = ((c0 & 0x01) << 30) |
            (((uint32_t) str[pos + 1] & 0x3f) << 24) |
            (((uint32_t) str[pos + 2] & 0x3f) << 18) |
            (((uint32_t) str[pos + 3] & 0x3f) << 12) |
            (((uint32_t) str[pos + 4] & 0x3f) << 6)  |
            ((uint32_t)  str[pos + 5] & 0x3f);
        return true;
...
{
    uint32_t page = (uint32_t) ch >> 8;
    if (page >= 256)
        return ch;

    uint32_t* normTable = WGL4NormalizationTables[page];
    if (normTable == NULL)
        return ch;

    return (wchar_t) normTable[(uint32_t) ch & 0xff];
}
...
size_t
ReplaceGreekLetterAbbr(char *dst, size_t dstSize, const char* src, size_t srcLength)
{
    if (src[0] >= 'A' && src[0] <= 'Z' &&
        src[1] >= 'A' && src[1] <= 'Z')
    {
        // Linear search through all letter abbreviations
        for (size_t i = 0; i < Greek::instance->nLetters; i++)
        {
            ...
            if (abbr[j] == '\0')
            {
                ...
                size_t utfGreekLength = strlen(utfGreek);

                size_t requiredLength = srcLength;
