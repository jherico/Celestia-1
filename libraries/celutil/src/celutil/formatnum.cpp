...
FormattedNumber::FormattedNumber(double v, uint32_t _precision, uint32_t _flags) :
    value(v), precision(_precision), flags(_flags) {
}
...
std::ostream& operator<<(std::ostream& out, const FormattedNumber& num) {
    ...
    if (num.flags & FormattedNumber::GroupThousands) {
        ...
        auto j = sizeof(obuf) - 1;
        auto i = strlen(buf);
        size_t digitCount = 0;
        if (decimalPosition != NULL) {
            size_t len = strlen(decimalPosition);
            j -= len;
            i -= len;
