...
int compareIgnoringCase(const string& s1, const string& s2) {
    ...
    return static_cast<int>(s2.size()) - static_cast<int>(s1.size());
}

int compareIgnoringCase(const string& s1, const string& s2, size_t n) {
    string::const_iterator i1 = s1.begin();
    string::const_iterator i2 = s2.begin();
...
bool CompareIgnoringCasePredicate::operator()(const string& s1, const string& s2) const {
