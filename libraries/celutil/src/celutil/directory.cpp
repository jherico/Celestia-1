...
bool Directory::enumFiles(EnumFilesHandler& handler, bool deep) {
    ...
    while (nextFile(filename)) {
        ...
        if (IsDirectory(pathname)) {
            if (deep) {
                auto dir = OpenDirectory(pathname);
                bool cont = true;
                if (dir != NULL) {
...
public:
    DirectoryImpl(const std::string& filename) : path(filename), itr(path) {}
...
DirectoryPtr OpenDirectory(const std::string& filename) {
    return std::make_shared<DirectoryImpl>(filename);
}
