...
bool UTF8Decode(const std::string& str, size_t pos, wchar_t& ch);
bool UTF8Decode(const char* str, size_t pos, size_t length, wchar_t& ch);
...
inline size_t UTF8EncodedSize(wchar_t ch)
{
    if (ch < 0x80)
        return 1;
    else if (ch < 0x800)
...
inline size_t UTF8EncodedSizeFromFirstByte(uint32_t ch)
{
    int charlen = 1;
...
size_t ReplaceGreekLetterAbbr(char* dst, size_t dstSize, const char* src, size_t srcLength);
