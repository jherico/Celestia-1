 ...
 *  is const.
 */
Material::Pointer Model::getMaterial(uint32_t index) const {
 ...
 */
uint32_t Model::addMaterial(const Material::Pointer& m) {
...
Mesh::Pointer Model::getMesh(uint32_t index) const {
...
uint32_t Model::addMesh(const Mesh::Pointer& m) {
    meshes.push_back(m);
    return static_cast<uint32_t>(meshes.size());
}

bool Model::pick(const Eigen::Vector3d& rayOrigin, const Eigen::Vector3d& rayDirection, Mesh::PickResult& result) const {
    double maxDistance = 1.0e30;
    double closest = maxDistance;
    Mesh::PickResult closestResult;

    for (const auto& mesh : meshes) {
        Mesh::PickResult localResult;
        if (mesh->pick(rayOrigin, rayDirection, result)) {
            if (localResult.distance < closest) {
                closestResult = localResult;
                closestResult.mesh = mesh;
                closest = localResult.distance;
            }
        }
    }
...
bool Model::pick(const Vector3d& rayOrigin, const Vector3d& rayDirection, double& distance) const {
 ...
 */
void Model::transform(const Vector3f& translation, float scale) {
    for (const auto& mesh : meshes) {
        mesh->transform(translation, scale);
    }
}

void Model::normalize(const Vector3f& centerOffset) {
...
static bool operator<(const Material::Color& c0, const Material::Color& c1) {
    if (c0.red() < c1.red())
        return true;
...
// materials.
static bool operator<(const Material& m0, const Material& m1) {
    ...
    for (uint32_t i = 0; i < Material::TextureSemanticMax; i++) {
...
static bool operator<(const IndexedMaterial& im0, const IndexedMaterial& im1) {
...
void Model::uniquifyMaterials() {
    ...
    vector<Material::Pointer> uniqueMaterials;
    vector<uint32_t> materialMap(materials.size());
    vector<uint32_t> duplicateMaterials;
...
void Model::determineOpacity() {
    for (const auto& material : materials) {
        if ((material->opacity > 0.01f && material->opacity < 1.0f) || material->blend == Material::AdditiveBlend) {
            opaque = false;
            return;
        }
    }
...
bool Model::usesTextureType(Material::TextureSemantic t) const {
...
static uint32_t getMeshMaterialIndex(const Mesh& mesh) {
...
Model::MeshComparator Model::OpacityComparator = [](const Mesh::Pointer& a, const Mesh::Pointer& b)->bool {
...
void Model::sortMeshes(const MeshComparator& comparator) {
    ...
    sort(meshes.begin(), meshes.end(), comparator);
}
