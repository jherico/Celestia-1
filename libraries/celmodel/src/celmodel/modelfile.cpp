...
using IncrementalStorage = storage::IncrementalStorage;
...
private:
    storage::IncrementalStorage::Pointer m_in;
...
public:
    ...
    VertexDataPointer loadVertices(const Mesh::VertexDescription& vertexDesc, uint32_t& vertexCount) override;
...
VertexDataPointer AsciiModelLoader::loadVertices(const Mesh::VertexDescription& vertexDesc, uint32_t& vertexCount) {
    ...
    vertexCount = (uint32_t)num;
    uint32_t vertexDataSize = vertexDesc.stride * vertexCount;
    ...
    uint32_t offset = 0;
    double data[4];
    for (uint32_t i = 0; i < vertexCount; i++, offset += vertexDesc.stride) {
        ...
        for (const auto& attribute : vertexDesc.attributes) {
            ...
            uint32_t base = offset + attribute.offset;
            if (fmt == Mesh::UByte4) {
...
Mesh::Pointer AsciiModelLoader::loadMesh() {
    ...
    while (tok.nextToken().isName() && tok.currentToken() != EndMeshToken) {
        ...
        if (tok.currentToken().integerValue() == -1) {
            ...
            materialIndex = (uint32_t)tok.currentToken().integerValue();
        }
        ...
        uint32_t indexCount = (uint32_t)tok.currentToken().integerValue();
        ...
        for (uint32_t i = 0; i < indexCount; i++) {
            ...
            uint32_t index = (uint32_t)tok.currentToken().integerValue();
...
static int32_t readUint(IncrementalStorage& in) {
    int32_t ret;
...
static int16_t readInt16(IncrementalStorage& in) {
    int16_t ret;
...
Mesh::Pointer BinaryModelLoader::loadMesh() {
    ...
    for (;;) {
        ...
        uint32_t materialIndex = readUint(in);
        uint32_t indexCount = readUint(in);
...
VertexDataPointer BinaryModelLoader::loadVertices(const Mesh::VertexDescription& vertexDesc, uint32_t& vertexCount) {
    ...
    for (uint32_t i = 0; i < vertexCount; i++, offset += vertexDesc.stride) {
        ...
        for (const auto& attr : vertexDesc.attributes) {
            uint32_t base = offset + attr.offset;
