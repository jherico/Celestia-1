...
Mesh::VertexDescription::VertexDescription(uint32_t _stride, const VertexAttributes& _attributes) :
...
void Mesh::VertexDescription::clearSemanticMap() {
    for (uint32_t i = 0; i < SemanticMax; i++) {
        semanticMap[i] = {};
    }
}
...
uint32_t Mesh::PrimitiveGroup::getPrimitiveCount() const {
    auto nIndices = static_cast<uint32_t>(indices.size());
    switch (prim) {
        case TriList:
            return nIndices / 3;
        case TriStrip:
...
void Mesh::setVertices(uint32_t _nVertices, const VertexDataPointer& vertexData) {
...
Mesh::PrimitiveGroup::ConstPointer Mesh::getGroup(uint32_t index) const {
...
Mesh::PrimitiveGroup::Pointer Mesh::getGroup(uint32_t index) {
    if (index >= groups.size()) {
        return nullptr;
    }
    return groups[index];
...
uint32_t Mesh::addGroup(const PrimitiveGroup::Pointer& group) {
    groups.push_back(group);
    return static_cast<uint32_t>(groups.size());
}

uint32_t Mesh::addGroup(PrimitiveGroupType prim, uint32_t materialIndex, IndexData& indexData) {
...
uint32_t Mesh::addGroup(PrimitiveGroupType prim, uint32_t materialIndex, const IndexData& indexData) {
    auto g = std::make_shared<PrimitiveGroup>();
    g->prim = prim;
    g->materialIndex = materialIndex;
    g->indices = indexData;
...
uint32_t Mesh::getGroupCount() const {
    return static_cast<uint32_t>(groups.size());
}
...
void Mesh::remapMaterials(const vector<uint32_t>& materialMap) {
    for (const auto& group : groups) {
...
bool Mesh::pick(const Vector3d& rayOrigin, const Vector3d& rayDirection, PickResult& result) const {
    ...
    uint32_t posOffset = vertexDesc.getAttribute(Position).offset;
...
AlignedBox<float, 3> Mesh::getBoundingBox() const {
    ...
    if (vertexDesc.getAttribute(PointSize).format == Float1) {
        ...
        for (uint32_t i = 0; i < nVertices; i++, vdata += vertexDesc.stride) {
...
uint32_t Mesh::getVertexAttributeSize(VertexAttributeFormat fmt) {
    switch (fmt) {
        case Float1:
        case UByte4:
            return 4;
