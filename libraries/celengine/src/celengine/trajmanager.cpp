...
static std::shared_ptr<TrajectoryManager> trajectoryManager;

static const char UniqueSuffixChar = '!';

const std::shared_ptr<TrajectoryManager>& GetTrajectoryManager() {
    if (trajectoryManager == NULL)
        trajectoryManager = std::make_shared<TrajectoryManager>("data");
...
string TrajectoryInfo::resolve(const string& baseDir) {
    ...
    sprintf(uniquifyingSuffix, "%c%u%u", UniqueSuffixChar, (uint32_t)interpolation, (uint32_t)precision);
...
Orbit::Pointer TrajectoryInfo::load(const string& filename) {
