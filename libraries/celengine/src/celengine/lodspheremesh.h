...
public:
    ...
    //            uint32_t attributes, const Frustum&, float pixWidth,
 ...
 private:
    ...
    //    int step;                 
    //    uint32_t attributes;  // vertex attributes
    ...
    uint32_t subtextures[MAX_SPHERE_MESH_TEXTURES];

    bool vertexBuffersInitialized;
    bool useVertexBuffers;
    int currentVB;
    uint32_t vertexBuffers[NUM_SPHERE_VERTEX_BUFFERS];
    //GLuint indexBuffer;
};
