...
public:
    LCGRandomGenerator() : previous(0) {}

    LCGRandomGenerator(uint64_t seed) : previous(seed) {}
     ...
     */
    int32_t randInt32() { return (int32_t)(randUint64() >> 16); }

    /*! Return a random integer between 0 and 2^32 - 1                           
     */
    uint32_t randUint32() { return (uint32_t)(randUint64() >> 16); }
...
Vector3f ConstantGenerator::generate(LCGRandomGenerator& /* gen */) const {
    return m_value;
}

Vector3f BoxGenerator::generate(LCGRandomGenerator& gen) const {
    return Vector3f(gen.randSfloat() * m_semiAxes.x(), gen.randSfloat() * m_semiAxes.y(), gen.randSfloat() * m_semiAxes.z()) +
           m_center;
}

Vector3f LineGenerator::generate(LCGRandomGenerator& gen) const {
    return m_origin + m_direction * gen.randFloat();
}

Vector3f EllipsoidSurfaceGenerator::generate(LCGRandomGenerator& gen) const {
...
Vector3f ConeGenerator::generate(LCGRandomGenerator& gen) const {
    float theta = (float)PI * gen.randSfloat();
...
Vector3f GaussianDiscGenerator::generate(LCGRandomGenerator& gen) const {
    float r1 = 0.0f;
    float r2 = 0.0f;
    float s = 0.0f;
...
ParticleEmitter::ParticleEmitter() :
    m_startTime(-numeric_limits<double>::infinity()), m_endTime(-numeric_limits<double>::infinity()),
    m_texture(InvalidResource), m_rate(1.0f), m_lifetime(1.0f), m_startColor(1.0f, 1.0f, 1.0f, 0.0f), m_startSize(1.0f),
    m_endColor(1.0f, 1.0f, 1.0f, 0.0f), m_endSize(1.0f), m_positionGenerator(NULL), m_velocityGenerator(NULL),
    m_acceleration(0.0f, 0.0f, 0.0f), m_nonZeroAcceleration(false), m_minRotationRate(0.0f), m_rotationRateVariance(0.0f),
    m_rotationEnabled(false), m_blendMode(cmod::Material::PremultipliedAlphaBlend) {
}

ParticleEmitter::~ParticleEmitter() {
}

void ParticleEmitter::setLifespan(double startTime, double endTime) {
    m_startTime = startTime;
    m_endTime = endTime;
}

void ParticleEmitter::setRotationRateRange(float minRate, float maxRate) {
    m_rotationEnabled = minRate != 0.0f || maxRate != 0.0f;
    m_minRotationRate = minRate;
    m_rotationRateVariance = maxRate - minRate;
}

void ParticleEmitter::setAcceleration(const Vector3f& acceleration) {
    m_acceleration = acceleration;
    m_nonZeroAcceleration = m_acceleration != Vector3f::Zero();
}

void ParticleEmitter::setBlendMode(cmod::Material::BlendMode blendMode) {
...
//                        uint32_t particleBufferCapacity) const
//{
//    double t = tsec;
...
void ParticleEmitter::createMaterial() {
    ...
    m_material.maps[0] = std::make_shared<CelestiaTextureResource>(m_texture);
}

ParticleSystem::ParticleSystem() : m_vertexDesc(NULL), m_vertexData(NULL), m_particleCapacity(0), m_particleCount(0) {
    ...
    std::vector<Mesh::VertexAttribute> attributes{
        Mesh::VertexAttribute(Mesh::Position, Mesh::Float3, offsetof(ParticleVertex, position)),
        Mesh::VertexAttribute(Mesh::Texture0, Mesh::Float2, offsetof(ParticleVertex, texCoord)),
        Mesh::VertexAttribute(Mesh::Color0, Mesh::UByte4, offsetof(ParticleVertex, color)),
    };
    m_vertexDesc = new Mesh::VertexDescription(sizeof(ParticleVertex), attributes);
}
...
bool ParticleSystem::pick(const Ray3d& /* r */, double& /* distance */) const {
    // Pick selection for particle systems not supported (because it's
    // not typically desirable.)
    return false;
}

bool ParticleSystem::isOpaque() const {
    return false;
}

void ParticleSystem::addEmitter(const ParticleEmitter::Pointer& emitter) {
