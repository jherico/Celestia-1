...
Observer::Observer() :
    simTime(0.0), position(0.0, 0.0, 0.0), orientation(Quaternionf::Identity()), velocity(0.0, 0.0, 0.0),
    angularVelocity(0.0, 0.0, 0.0), frame(NULL), realTime(0.0), targetSpeed(0.0), targetVelocity(0.0, 0.0, 0.0),
    initialVelocity(0.0, 0.0, 0.0), beginAccelTime(0.0), observerMode(Free), trackingOrientation(Quaternionf::Identity()),
    fov((float)(PI / 4.0)), reverseFlag(false), locationFilter(~0u) {
...
Observer::Observer(const Observer& o) :
    simTime(o.simTime), position(o.position), orientation(o.orientation), velocity(o.velocity),
    angularVelocity(o.angularVelocity), frame(NULL), realTime(o.realTime), targetSpeed(o.targetSpeed),
    targetVelocity(o.targetVelocity), beginAccelTime(o.beginAccelTime), observerMode(o.observerMode), journey(o.journey),
    trackObject(o.trackObject), trackingOrientation(o.trackingOrientation), fov(o.fov), reverseFlag(o.reverseFlag),
    locationFilter(o.locationFilter), displayedSurface(o.displayedSurface) {
    frame = std::make_shared<ObserverFrame>(*o.frame);
    updateUniversal();
}
...
// the celmath package.
template <class T>
static Quaternion<T> lookAt(Matrix<T, 3, 1> from, Matrix<T, 3, 1> to, Matrix<T, 3, 1> up) {
...
#ifdef CELVEC
template <class T>
static Quat<T> lookAt(Point3<T> from, Point3<T> to, Vector3<T> up) {
    Vector3<T> n = to - from;
    n.normalize();
    Vector3<T> v = n ^ up;
    v.normalize();
    ...
    if (observerMode == Travelling) {
        ...
        // and there will be no coasting at constant velocity.
        {
            ...
            if (u < journey.accelTime) {
                ...
                x = exp(journey.expFactor * journey.accelTime) * (journey.expFactor * (u - journey.accelTime) + 1) - 1;
            }
        ...
        if (t >= journey.startInterpolation && t < journey.endInterpolation) {
            ...
            if (journey.traj == CircularOrbit) {
                ...
                v = pow(sin((t - journey.startInterpolation) / (journey.endInterpolation - journey.startInterpolation) * PI /
                            2),
                        2);
            }
...
void Observer::computeGotoParameters(const Selection& destination,
    ...
    pair<double, double> sol = solve_bisection(TravelExpFunc(distance, jparams.accelTime), 0.0001, 100.0, 1e-10);
...
void Observer::computeCenterParameters(const Selection& destination, JourneyParams& jparams, double centerTime) {
    UniversalCoord targetPosition = destination.getPosition(getTime());
...
void Observer::computeCenterCOParameters(const Selection& destination, JourneyParams& jparams, double centerTime) {
    jparams.duration = centerTime;
    jparams.startTime = realTime;
...
// the goto parameters.
void Observer::convertFrameCoordinates(const ObserverFramePtr& newFrame) {
    double now = getTime();
...
void Observer::setFrame(const ObserverFrame& f) {
    if (frame.get() != &f) {
        auto newFrame = std::make_shared<ObserverFrame>(f);
        if (frame) {
            convertFrameCoordinates(newFrame);
        }
        frame = newFrame;
 ...
 */
const ObserverFramePtr& Observer::getFrame() const {
    return frame;
}
...
void Observer::gotoJourney(const JourneyParams& params) {
    ...
    pair<double, double> sol = solve_bisection(TravelExpFunc(distance, journey.accelTime), 0.0001, 100.0, 1e-10);
...
static double getPreferredDistance(const Selection& selection) {
    switch (selection.getType()) {
        ...
        case Selection::Type_Location: {
            ...
            return max(min(selection.location()->getSize() * 50.0, maxDist), 1.0);
        }
...
void Observer::gotoSelection(const Selection& selection,
    ...
    if (!selection.empty()) {
        ...
        computeGotoParameters(selection, journey, gotoTime, startInter, endInter, v * -(orbitDistance / distance),
                              ObserverFrame::Universal, up, upFrame);
        observerMode = Travelling;
    }
}
...
void Observer::gotoSelectionGC(const Selection& selection,
    ...
    if (!selection.empty()) {
        ...
        computeGotoParametersGC(selection, journey, gotoTime,
                                //startInter, endInter,
                                0.25, 0.75, v * (orbitDistance / distanceToCenter), ObserverFrame::Universal, up, upFrame,
                                centerObj);
        observerMode = Travelling;
    }
}
...
void Observer::gotoSelection(const Selection& selection,
    ...
    if (!selection.empty()) {
        ...
        computeGotoParameters(selection, journey, gotoTime, 0.25, 0.75, v * -distance, ObserverFrame::Universal, up, upFrame);
        observerMode = Travelling;
    }
}
...
void Observer::gotoSelectionGC(const Selection& selection,
    ...
    if (!selection.empty()) {
        ...
        computeGotoParametersGC(selection, journey, gotoTime, 0.25, 0.75, v * -distance, ObserverFrame::Universal, up, upFrame,
                                centerObj);
        observerMode = Travelling;
    }
}
...
void Observer::gotoSelectionLongLat(const Selection& selection,
    ...
    if (!selection.empty()) {
        ...
        computeGotoParameters(selection, journey, gotoTime, 0.25, 0.75, Vector3d(x, y, z) * distance, ObserverFrame::BodyFixed,
                              up, ObserverFrame::BodyFixed);
        observerMode = Travelling;
    }
}

void Observer::gotoLocation(const UniversalCoord& toPosition, const Quaterniond& toOrientation, double duration) {
    journey.startTime = realTime;
...
void Observer::getSelectionLongLat(const Selection& selection, double& distance, double& longitude, double& latitude) {
...
void Observer::geosynchronousFollow(const Selection& selection) {
    if (selection.body() != NULL || selection.location() != NULL || selection.star() != NULL) {
 ...
 *  reference frame.
 */
ObserverFrame::ObserverFrame() : coordSys(Universal), frame(createFrame(Universal, Selection(), Selection())) {
}
 ...
 */
ObserverFrame::ObserverFrame(CoordinateSystem _coordSys, const Selection& _refObject, const Selection& _targetObject) :
    coordSys(_coordSys), frame(createFrame(_coordSys, _refObject, _targetObject)), targetObject(_targetObject) {
}
 ...
 */
ObserverFrame::ObserverFrame(const ReferenceFramePtr& f) : coordSys(Unknown), frame(f) {
}

/*! Copy constructor. */
ObserverFrame::ObserverFrame(const ObserverFrame& f) : coordSys(f.coordSys), frame(f.frame), targetObject(f.targetObject) {
}
...
UniversalCoord ObserverFrame::convertFromUniversal(const UniversalCoord& uc, double tjd) const {
    return frame->convertFromUniversal(uc, tjd);
}

UniversalCoord ObserverFrame::convertToUniversal(const UniversalCoord& uc, double tjd) const {
    return frame->convertToUniversal(uc, tjd);
}

Quaterniond ObserverFrame::convertFromUniversal(const Quaterniond& q, double tjd) const {
    return frame->convertFromUniversal(q, tjd);
}

Quaterniond ObserverFrame::convertToUniversal(const Quaterniond& q, double tjd) const {
    return frame->convertToUniversal(q, tjd);
}

/*! Convert a position from one frame to another.
 */
UniversalCoord ObserverFrame::convert(const ObserverFramePtr& fromFrame,
                                      const ObserverFramePtr& toFrame,
...
*/
Quaterniond ObserverFrame::convert(const ObserverFramePtr& fromFrame,
                                   const ObserverFramePtr& toFrame,
                                   const Quaterniond& q,
                                   double t) {
    // Perform the conversion fromFrame -> universal -> toFrame
...
ReferenceFrameConstPtr ObserverFrame::createFrame(CoordinateSystem _coordSys,
    ...
    switch (_coordSys) {
        ...
        case Chase: {
            return std::make_shared<TwoVectorFrame>(_refObject,
                                                    FrameVector::createRelativeVelocityVector(_refObject, _refObject.parent()),
                                                    1,
                                                    FrameVector::createRelativePositionVector(_refObject, _refObject.parent()),
                                                    2);
        }

        case PhaseLock_Old: {
            FrameVector rotAxis(
                FrameVector::createConstantVector(Vector3d::UnitY(),
        ...
        case Chase_Old: {
            ...
            return std::make_shared<TwoVectorFrame>(_refObject,
                                                    FrameVector::createRelativeVelocityVector(_refObject.parent(), _refObject),
                                                    3, rotAxis, 2);
        }
