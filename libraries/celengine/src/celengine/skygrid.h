...
public:
    ...
    void render(Renderer& renderer, const Observer& observer);

    Eigen::Quaterniond orientation() const { return m_orientation; }

    void setOrientation(const Eigen::Quaterniond& orientation) { m_orientation = orientation; }

    Color lineColor() const { return m_lineColor; }

    void setLineColor(Color lineColor) { m_lineColor = lineColor; }

    Color labelColor() const { return m_labelColor; }

    void setLabelColor(Color labelColor) { m_labelColor = labelColor; }

    //! Get the units of longitude (hours or degrees)
    LongitudeUnits longitudeUnits() const { return m_longitudeUnits; }

    //! Set whether longitude is measured in hours or degrees
    void setLongitudeUnits(LongitudeUnits longitudeUnits) { m_longitudeUnits = longitudeUnits; }

    //! Get the direction of increasing longitude
    LongitudeDirection longitudeDirection() const { return m_longitudeDirection; }

    //! Set the direction of increasing longitude (clockwise or counterclockwise)
    void setLongitudeDirection(LongitudeDirection longitudeDirection) { m_longitudeDirection = longitudeDirection; }
