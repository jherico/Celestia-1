...
static const uint32_t GLOBULAR_POINTS = 8192;
...
Globular::Globular() : detail(1.0f), customTmpName(NULL), form(NULL), r_c(R_c_ref), c(C_ref), tidalRadius(0.0f) {
    recomputeTidalRadius();
}

uint32_t Globular::cSlot(float conc) const {
    ...
    return (uint32_t)floor((conc - MinC) / BinWidth);
}
...
bool Globular::pick(const Ray3d& ray, double& distanceToPicker, double& cosAngleToBoundCenter) const {
    if (!isVisible())
        return false;
    /*
     ...
     * when they are drawn. 
     */
    Vec3d ellipsoidAxes(getRadius() * (form->scale.x + RADIUS_CORRECTION), getRadius() * (form->scale.y + RADIUS_CORRECTION),
    ...
    return testIntersection(Ray3d(ray.origin - p, ray.direction).transform(getOrientation().cast<double>().toRotationMatrix()),
                            Ellipsoidd(ellipsoidAxes), distanceToPicker, cosAngleToBoundCenter);
}

bool Globular::load(const HashPtr& params, const string& resPath) {
    ...
    if (params->getAngle("CoreRadius", coreRadius, 1.0 / MINUTES_PER_DEG)) {
        r_c = static_cast<float>(coreRadius);
        setCoreRadius(r_c);
    }
...
//    uint32_t nPoints =
//                 (uint32_t) (points->size() * clamp(getDetail()));
//
...
uint32_t Globular::getRenderMask() const {
    return Renderer::ShowGlobulars;
}

uint32_t Globular::getLabelMask() const {
