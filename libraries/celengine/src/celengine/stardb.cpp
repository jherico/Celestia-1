...
using StarBooleanPredicate = std::function<bool(const Star& a, const Star& b)>;

// Used to sort stars by catalog number

StarBooleanPredicate CatalogNumberOrderingPredicate = [](const Star& star0, const Star& star1) -> bool {
    return (star0.getCatalogNumber() < star1.getCatalogNumber());
};

StarBooleanPredicate CatalogNumberEquivalencePredicate = [](const Star& star0, const Star& star1) -> bool {
    return (star0.getCatalogNumber() == star1.getCatalogNumber());
};

using StarPtrBooleanPredicate = std::function<bool(const StarPtr& a, const StarPtr& b)>;

// Used to sort star pointers by catalog number
StarPtrBooleanPredicate PtrCatalogNumberOrderingPredicate = [](const StarPtr& star0, const StarPtr& star1) -> bool {
    return (star0->getCatalogNumber() < star1->getCatalogNumber());
};

static bool parseSimpleCatalogNumber(const string& name, const string& prefix, uint32_t& catalogNumber) {
    char extra[4];
...
static bool parseHIPPARCOSCatalogNumber(const string& name, uint32_t& catalogNumber) {
    return parseSimpleCatalogNumber(name, HIPPARCOSCatalogPrefix, catalogNumber);
}

static bool parseHDCatalogNumber(const string& name, uint32_t& catalogNumber) {
    return parseSimpleCatalogNumber(name, HDCatalogPrefix, catalogNumber);
}

static bool parseTychoCatalogNumber(const string& name, uint32_t& catalogNumber) {
    if (compareIgnoringCase(name, TychoCatalogPrefix, TychoCatalogPrefix.length()) == 0) {
        uint32_t tyc1 = 0, tyc2 = 0, tyc3 = 0;
...
static bool parseCelestiaCatalogNumber(const string& name, uint32_t& catalogNumber) {
    char extra[4];
...
StarDatabase::StarDatabase() : namesDB(NULL), octreeRoot(NULL), nextAutoCatalogNumber(0xfffffffe) {
...
StarPtr StarDatabase::find(uint32_t catalogNumber) const {
    auto refStar = std::make_shared<Star>();
    refStar->setCatalogNumber(catalogNumber);

    auto iter = lower_bound(catalogNumberIndex.begin(), catalogNumberIndex.end(), refStar, PtrCatalogNumberOrderingPredicate);

    if (iter != catalogNumberIndex.end() && (*iter)->getCatalogNumber() == catalogNumber)
        return *iter;
    else
        return NULL;
}
...
uint32_t StarDatabase::findCatalogNumberByName(const string& name) const {
    ...
    } else if (parseSimpleCatalogNumber(name, SAOCatalogPrefix, catalogNumber)) {
...
StarPtr StarDatabase::find(const string& name) const {
...
uint32_t StarDatabase::crossIndex(const Catalog catalog, const uint32_t celCatalogNumber) const {
    ...
    auto xindex = crossIndexes[catalog];
    if (xindex == NULL)
        return Star::InvalidCatalogNumber;
    ...
    for (const auto entry : *xindex) {
        if (celCatalogNumber == entry.celCatalogNumber)
...
uint32_t StarDatabase::searchCrossIndexForCatalogNumber(const Catalog catalog, const uint32_t number) const {
    ...
    CrossIndex::iterator iter = lower_bound(xindex->begin(), xindex->end(), xindexEnt);
...
StarPtr StarDatabase::searchCrossIndex(const Catalog catalog, const uint32_t number) const {
...
static void catalogNumberToString(uint32_t catalogNumber, char* buf, uint32_t bufSize) {
...
void StarDatabase::findCloseStars(StarHandler& starHandler, const Vector3f& position, float radius) const {
    octreeRoot->processCloseObjects(starHandler, position, radius, STAR_OCTREE_ROOT_SIZE);
}

const StarNameDatabase::Pointer& StarDatabase::getNameDatabase() const {
    return namesDB;
}

void StarDatabase::setNameDatabase(const StarNameDatabase::Pointer& _namesDB) {
    namesDB = _namesDB;
}
...
bool StarDatabase::loadCrossIndex(const Catalog catalog, istream& in) {
    ...
    crossIndexes[catalog].reset();

    // Verify that the star database file has a correct header
    {
        auto headerLength = strlen(CROSSINDEX_FILE_HEADER);
    ...
    auto xindex = std::make_shared<CrossIndex>();
    uint32_t record = 0;
    for (;;) {
        CrossIndexEntry ent;
...
bool StarDatabase::loadBinary(istream& in) {
    ...
    // Verify that the star database file has a correct header
    {
        auto headerLength = strlen(FILE_HEADER);
        char* header = new char[headerLength];
        in.read(header, headerLength);
...
void StarDatabase::finish() {
    ...
    binFileCatalogNumberIndex.clear();
    ...
    // to be addressed.
    for (vector<BarycenterUsage>::const_iterator iter = barycenters.begin(); iter != barycenters.end(); iter++) {
        auto star = find(iter->catNo);
        auto barycenter = find(iter->barycenterCatNo);
        assert(star != NULL);
...
static void stcError(const Tokenizer& tok, const string& msg) {
 ...
 */
bool StarDatabase::createStar(const StarPtr& star,
                              StcDisposition disposition,
                              uint32_t catalogNumber,
                              const HashPtr& starData,
                              const string& path,
                              bool isBarycenter) {
    StarDetails::Pointer details = NULL;
    string spectralType;
    ...
    if (disposition == ModifyStar) {
        auto existingDetails = star->getDetails();
    ...
    auto rm = CreateRotationModel(starData, path, 1.0);
    ...
    auto orbit = CreateOrbit(Selection(), starData, path, true);

    if (hasTexture || hasModel || orbit != NULL || hasSemiAxes || hasRadius || hasTemperature || hasBolometricCorrection ||
        hasRotationModel || hasInfoURL) {
        ...
        if (hasTemperature) {
            ...
            if (!hasBolometricCorrection) {
                ...
                double bc = -8.499 * pow(logT, 4) + 13.421 * pow(logT, 3) - 8.131 * logT * logT - 3.901 * logT - 0.438;
        ...
        if (orbit != NULL) {
            ...
            if (barycenterDefined) {
                if (barycenterCatNo != Star::InvalidCatalogNumber) {
                    ...
                    auto barycenter = findWhileLoading(barycenterCatNo);
                    if (barycenter != NULL) {
                        hasBarycenter = true;
...
bool StarDatabase::load(istream& in, const string& resourcePath) {
    ...
    while (tokenizer.nextToken() != Tokenizer::TokenEnd) {
        ...
        auto starDataValue = parser.readValue();
        ...
        const auto& starData = starDataValue->getHash();
...
void StarDatabase::buildOctree() {
    ...
    for (vector<OctreeLevelStatistics>::const_iterator iter = stats.begin(); iter != stats.end(); ++iter) {
        ...
        clog << "Level " << level << ", " << STAR_OCTREE_ROOT_SIZE / pow(2.0, (double)level) << "ly, " << iter->nodeCount
 ...
 */
StarPtr StarDatabase::findWhileLoading(uint32_t catalogNumber) const {
    // First check for stars loaded from the binary database
    if (!binFileCatalogNumberIndex.empty()) {
        auto refStar = std::make_shared<Star>();
        refStar->setCatalogNumber(catalogNumber);
        auto begin = binFileCatalogNumberIndex.begin();
        auto end = binFileCatalogNumberIndex.end();
        auto iter = lower_bound(begin, end, refStar, PtrCatalogNumberOrderingPredicate);
        if (iter != end && (*iter)->getCatalogNumber() == catalogNumber)
            return *iter;
    }
