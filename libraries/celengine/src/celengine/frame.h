...
public:
    using Pointer = std::shared_ptr<ReferenceFrame>;
    ...
    uint32_t nestingDepth(uint32_t maxDepth, FrameType) const;

    virtual uint32_t nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType frameType) const = 0;
...
public:
    ...
    Eigen::Quaterniond getOrientation(double /* tjd */) const { return Eigen::Quaterniond::Identity(); }

    virtual bool isInertial() const;

    virtual uint32_t nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType frameType) const;
};
...
class FrameVector {
public:
    using Pointer = std::shared_ptr<FrameVector>;
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
     ...
     *  frames.
     */
    uint32_t nestingDepth(uint32_t depth, uint32_t maxDepth) const;
    ...
    static FrameVector createRelativePositionVector(const Selection& _observer, const Selection& _target);
    static FrameVector createRelativeVelocityVector(const Selection& _observer, const Selection& _target);
    static FrameVector createConstantVector(const Eigen::Vector3d& _vec, const ReferenceFrameConstPtr& _frame);
...
private:
    ...
    ReferenceFrameConstPtr frame;  // frame for constant vector
};
...
public:
     ...
     */
    TwoVectorFrame(Selection center, const FrameVector& prim, int primAxis, const FrameVector& sec, int secAxis);
