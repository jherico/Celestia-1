...
Universe::Universe() :
    starCatalog(NULL), dsoCatalog(NULL), solarSystemCatalog(NULL), asterisms(NULL), boundaries(NULL), markers(NULL) {
}
...
SolarSystemPtr Universe::getSolarSystem(const Selection& sel) const {
    switch (sel.getType()) {
        case Selection::Type_Star:
            return getSolarSystem(sel.star());

        case Selection::Type_Body: {
            auto system = sel.body()->getSystem();
            while (system != NULL) {
                auto parent = system->getPrimaryBody();
                if (parent != NULL)
...
SolarSystemPtr Universe::createSolarSystem(const StarPtr& star) const {
    auto solarSystem = getSolarSystem(star);
    if (solarSystem != NULL)
        return solarSystem;

    solarSystem = std::make_shared<SolarSystem>(star);
    solarSystemCatalog->insert(SolarSystemCatalog::value_type(star->getCatalogNumber(), solarSystem));
...
void Universe::markObject(const Selection& sel,
    ...
    std::remove_if(markers.begin(), markers.end(), [&](const Marker& marker) {
        if (marker.object() == sel) {
    ...
    markers.emplace_back(sel, priority, rep, occludable, sizing);
}
...
bool Universe::isMarked(const Selection& sel, int priority) const {
    return std::any_of(markers.begin(), markers.end(), [&](const Marker& marker) {
        return (marker.object() == sel) && marker.priority() >= priority;
    });
}

class ClosestStarFinder : public StarHandler {
public:
    ClosestStarFinder(float _maxDistance, const UniverseConstPtr& _universe);
    ~ClosestStarFinder(){};
...
ClosestStarFinder::ClosestStarFinder(float _maxDistance, const UniverseConstPtr& _universe) :
    maxDistance(_maxDistance), closestDistance(_maxDistance), closestStar(NULL), universe(_universe), withPlanets(false) {
}
...
void NearStarFinder::process(const StarPtr& star, float distance, float) {
...
static bool ApproxPlanetPickTraversal(const BodyPtr& body, void* info) {
...
static bool ExactPlanetPickTraversal(const BodyPtr& body, void* info) {
    ...
    if (body->isVisible() && body->extant(pickInfo->jd) && body->isClickable() &&
        ...
        if (sinAngle2 < sin(PI / 4.0) && distance > 0.0 && distance <= pickInfo->closestDistance) {
...
static bool traverseFrameTree(const std::shared_ptr<FrameTree>& frameTree,
                              double tdb,
                              PlanetarySystem::TraversalFunc func,
                              void* info) {
    for (uint32_t i = 0; i < frameTree->childCount(); i++) {
...
Selection Universe::pickPlanet(SolarSystem& solarSystem,
    ...
    if (pickInfo.closestBody != NULL) {
        // Retain that body
        auto closestBody = pickInfo.closestBody;
        ...
        if ((pickInfo.sinAngle2Closest <= sinTol2) && (pickInfo.closestDistance > pickInfo.closestApproxDistance))
...
public:
    ...
    void process(const StarPtr&, float, float);
...
StarPicker::StarPicker(const Vector3f& _pickOrigin, const Vector3f& _pickRay, double _when, float angle) :
    pickedStar(NULL), pickOrigin(_pickOrigin), pickRay(_pickRay), sinAngle2Closest(std::max(sin(angle / 2.0), ANGULAR_RES)),
    when(_when) {
}

void StarPicker::process(const StarPtr& star, float, float) {
    ...
    if (orbitalRadius != 0.0f) {
        ...
        if (testIntersection(Ray3f(Vector3f::Zero(), pickRay), Spheref(relativeStarPos, orbitalRadius * 2.0f), distance)) {
...
public:
    CloseStarPicker(const UniversalCoord& pos, const Vector3f& dir, double t, float _maxDistance, float angle);
    ~CloseStarPicker(){};
    void process(const StarPtr& star, float distance, float appMag) override;
...
CloseStarPicker::CloseStarPicker(const UniversalCoord& pos, const Vector3f& dir, double t, float _maxDistance, float angle) :
    pickOrigin(pos), pickDir(dir), now(t), maxDistance(_maxDistance), closestStar(NULL), closestDistance(0.0f),
    sinAngle2Closest(std::max(sin(angle / 2.0), ANGULAR_RES)) {
}

void CloseStarPicker::process(const StarPtr& star, float lowPrecDistance, float) {
    ...
    if (testIntersection(Ray3f(Vector3f::Zero(), pickDir), Spheref(starDir, star->getRadius()), distance)) {
        if (distance > 0.0f) {
        ...
        if (sinAngle2 <= sinAngle2Closest && (closestStar == NULL || distance < closestDistance)) {
            closestStar = star;
            closestDistance = distance;
            sinAngle2Closest = std::max(sinAngle2, ANGULAR_RES);
        }
...
Selection Universe::pickStar(const UniversalCoord& origin,
    ...
    if (closePicker.closestStar != NULL)
        return Selection(closePicker.closestStar);
...
public:
    ...
    void process(const DeepSkyObjectPtr&, double, float) override;
...
public:
    ...
    DeepSkyObjectPtr pickedDSO;
    double sinAngle2Closest;
};

DSOPicker::DSOPicker(const Vector3d& pickOrigin, const Vector3d& pickDir, int renderFlags, float angle) :
    pickOrigin(pickOrigin), pickDir(pickDir), renderFlags(renderFlags), pickedDSO(NULL),
    sinAngle2Closest(std::max(sin(angle / 2.0), ANGULAR_RES)) {
}

void DSOPicker::process(const DeepSkyObjectPtr& dso, double, float) {
    ...
    if (testIntersection(Ray3d(Vector3d::Zero(), pickDir), Sphered(relativeDSOPos, (double)dso->getRadius()), distance2)) {
...
public:
    CloseDSOPicker(const Vector3d& pos, const Vector3d& dir, int renderFlags, double maxDistance, float);
    ~CloseDSOPicker(){};

    void process(const DeepSkyObjectPtr& dso, double distance, float appMag) override;
...
public:
    ...
    DeepSkyObjectPtr closestDSO;
    double largestCosAngle;
};

CloseDSOPicker::CloseDSOPicker(const Vector3d& pos, const Vector3d& dir, int renderFlags, double maxDistance, float) :
    pickOrigin(pos), pickDir(dir), renderFlags(renderFlags), maxDistance(maxDistance), closestDSO(NULL), largestCosAngle(-2.0) {
}

void CloseDSOPicker::process(const DeepSkyObjectPtr& dso, double distance, float) {
    ...
    if (dso->pick(Ray3d(pickOrigin, pickDir), distanceToPicker, cosAngleToBoundCenter)) {
        ...
        if ((pickOrigin - dso->getPosition()).norm() > dso->getRadius() && cosAngleToBoundCenter > largestCosAngle) {
            closestDSO = dso;
...
Selection Universe::pickDeepSkyObject(const UniversalCoord& origin,
    ...
    if (closePicker.closestDSO != NULL) {
        return Selection(closePicker.closestDSO);
    }
    ...
    dsoCatalog->findVisibleDSOs(picker, orig, rotation.conjugate(), tolerance, 1.0f, faintestMag);
    if (picker.pickedDSO != NULL)
        return Selection(picker.pickedDSO);
    else
        return Selection();
}
...
Selection Universe::pick(const UniversalCoord& origin,
    ...
    if (renderFlags & Renderer::ShowPlanets) {
        ...
        for (auto iter = closeStars.begin(); iter != closeStars.end(); iter++) {
            auto solarSystem = getSolarSystem(*iter);
            if (solarSystem != NULL) {
                sel = pickPlanet(*solarSystem, origin, direction, when, faintestMag, tolerance);
                if (!sel.empty())
                    break;
            }
        }
...
Selection Universe::findChildObject(const Selection& sel, const string& name, bool i18n) const {
    switch (sel.getType()) {
        case Selection::Type_Star: {
            auto sys = getSolarSystem(sel.star());
            if (sys != NULL) {
                auto planets = sys->getPlanets();
                if (planets != NULL)
        ...
        case Selection::Type_Body: {
            ...
            auto sats = sel.body()->getSatellites();
            if (sats != NULL) {
                auto body = sats->find(name, false, i18n);
                if (body != NULL)
                    return Selection(body);
            }

            // If a satellite wasn't found, check this object's locations
            auto loc = sel.body()->findLocation(name, i18n);
            if (loc != NULL)
...
Selection Universe::findObjectInContext(const Selection& sel, const string& name, bool i18n) const {
    ...
    auto sys = getSolarSystem(sel);
    if (sys != NULL) {
        auto planets = sys->getPlanets();
        if (planets != NULL) {
            auto body = planets->find(name, true, i18n);
            if (body != NULL)
                return Selection(body);
        }
    }
    ...
    // ...and then locations.
    if (contextBody != NULL) {
        auto loc = contextBody->findLocation(name, i18n);
        if (loc != NULL)
            return Selection(loc);
    }
...
//      to us to search.
Selection Universe::find(const string& s, Selection* contexts, int nContexts, bool i18n) const {
    if (starCatalog != NULL) {
        auto star = starCatalog->find(s);
        if (star != NULL)
            return Selection(star);
    }

    if (dsoCatalog != NULL) {
        auto dso = dsoCatalog->find(s);
        if (dso != NULL)
...
Selection Universe::findPath(const string& s, Selection contexts[], int nContexts, bool i18n) const {
...
vector<string> Universe::getCompletion(const string& s, Selection* contexts, int nContexts, bool withLocations) {
    vector<string> completion;
    auto s_length = UTF8Length(s);

    // Solar bodies first:
    for (int i = 0; i < nContexts; i++) {
        if (withLocations && contexts[i].getType() == Selection::Type_Body) {
            const auto& locations = contexts[i].body()->getLocations();
        ...
        auto sys = getSolarSystem(contexts[i]);
        if (sys != NULL) {
            auto planets = sys->getPlanets();
            if (planets != NULL) {
...
vector<string> Universe::getCompletionPath(const string& s, Selection* contexts, int nContexts, bool withLocations) {
    vector<string> completion;
    ...
    PlanetarySystemPtr worlds = NULL;
    if (sel.getType() == Selection::Type_Body) {
        worlds = sel.body()->getSatellites();
        const auto& locations = sel.body()->getLocations();
        if (!locations.empty() && withLocations) {
            string search = s.substr(pos + 1);
            for (auto iter = locations.begin(); iter != locations.end(); iter++) {
                if (!UTF8StringCompare(search, (*iter)->getName(true), search.length())) {
                    locationCompletion.push_back((*iter)->getName(true));
                }
            }
        }
    } else if (sel.getType() == Selection::Type_Star) {
        auto ssys = getSolarSystem(sel.star());
        if (ssys != NULL)
...
SolarSystemPtr Universe::getNearestSolarSystem(const UniversalCoord& position) const {
    Vector3f pos = position.toLy().cast<float>();
    ClosestStarFinder closestFinder(1.0f, shared_from_this());
