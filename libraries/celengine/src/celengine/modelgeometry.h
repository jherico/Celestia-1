...
{
 public:
    ModelGeometry(cmod::Model::Pointer model);
    ~ModelGeometry();
     ...
     *  distance unmodified.
     */
    bool pick(const Ray3d& r, double& distance) const override;

    //! Render the model in the current OpenGL context
    //virtual void render(RenderContext&, double t = 0.0);

    bool usesTextureType(cmod::Material::TextureSemantic) const override;
