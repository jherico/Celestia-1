...
static Model::Pointer LoadCelestiaMesh(const string& filename);
...
public:
    CelestiaTextureLoader(const std::string& texturePath) : m_texturePath(texturePath) {}

    ~CelestiaTextureLoader() {}

    Material::TextureResource::Pointer loadTexture(const std::string& name) {
        ResourceHandle tex = GetTextureManager()->getHandle(TextureInfo(name, m_texturePath, TextureInfo::WrapTexture));
        return std::make_shared<CelestiaTextureResource>(tex);
    }
...
string GeometryInfo::resolve(const string& baseDir) {
    ...
    sprintf(uniquifyingSuffix, "%c%f,%f,%f,%f,%d", UniqueSuffixChar, center.x(), center.y(), center.z(), scale,
            (int)isNormalized);
...
Geometry::Pointer GeometryInfo::load(const string& resolvedFilename) {
    ...
    if (fileType == Content_3DStudio) {
        ...
        if (in.good()) {
            auto textureLoader = std::make_shared<CelestiaTextureLoader>(path);
            model = LoadModel(filename, textureLoader);
            if (model != NULL) {
                if (isNormalized)
                    model->normalize(center);
                else
    ...
    if (model != NULL) {
        ...
        clog << _("   Model statistics: ") << model->getVertexCount() << _(" vertices, ") << model->getPrimitiveCount()
             << _(" primitives, ") << originalMaterialCount << _(" materials ") << "(" << model->getMaterialCount()
             << _(" unique)\n");

        return std::make_shared<ModelGeometry>(model);
    } else {
...
Model::Pointer LoadCelestiaMesh(const string& filename) {
    ...
    if (tokenizer.getStringValue() != "SphereDisplacementMesh") {
        DPRINTF(0, "%s: Unrecognized mesh type %s.\n", filename.c_str(), tokenizer.getStringValue().c_str());
        return NULL;
    }

    auto meshDefValue = parser.readValue();
    ...
    const auto& meshDef = meshDefValue->getHash();
    ...
    SphereMesh::Pointer sphereMesh = std::make_shared<SphereMesh>(params.size, (int)params.rings, (int)params.slices, NoiseDisplacementFunc, (void*)&params);
    Mesh::Pointer mesh = sphereMesh->convertToMesh();
    model->addMesh(mesh);
    return model;
}

#if 0
static Mesh* ConvertTriangleMesh(M3DTriangleMesh& mesh, const M3DScene& scene) {
    ...
    uint32_t floatsPerVertex = vertexSize / sizeof(float);
    ...
    for (uint32_t i = 0; i < mesh.getMeshMaterialGroupCount(); ++i) {
        ...
        uint32_t* indices = new uint32_t[nMatGroupFaces * 3];
...
static Material::Color toMaterialColor(const Color& c) {
...
static Model* Convert3DSModel(const M3DScene& scene, const string& texPath) {
    ...
    for (uint32_t i = 0; i < scene.getMaterialCount(); i++) {
        ...
        if (!material->getTextureMap().empty()) {
            ResourceHandle tex =
                GetTextureManager()->getHandle(TextureInfo(material->getTextureMap(), texPath, TextureInfo::WrapTexture));
    ...
    for (uint32_t i = 0; i < scene.getModelCount(); i++) {
        ...
        if (model3ds != NULL) {
            for (uint32_t j = 0; j < model3ds->getTriMeshCount(); j++) {
