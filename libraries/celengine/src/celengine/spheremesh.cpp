...
SphereMesh::SphereMesh(const Vector3f& size, const DisplacementMap& dispmap, float height) {
...
SphereMesh::SphereMesh(const Vector3f& size, int _nRings, int _nSlices, DisplacementMapFunc func, void* info) {
    createSphere(1.0f, _nRings, _nSlices);
    scale(size);
    displace(func, info);
    generateNormals();
...
void SphereMesh::displace(const DisplacementMap& dispmap, float height) {
...
Mesh::Pointer SphereMesh::convertToMesh() const {
    uint32_t stride = 32;
    Mesh::VertexAttributes attributes{
        Mesh::VertexAttribute(Mesh::Position, Mesh::Float3, 0),
        Mesh::VertexAttribute(Mesh::Normal, Mesh::Float3, 12),
        Mesh::VertexAttribute(Mesh::Texture0, Mesh::Float2, 24),
    };
    Mesh::Pointer mesh = std::make_shared<Mesh>();
    mesh->setVertexDescription(Mesh::VertexDescription(stride, attributes));

    // Copy the vertex data from the separate position, normal, and texture coordinate
    // arrays into a single array.
    VertexDataPointer vertexDataPtr = std::make_shared<VertexData>();
    vertexDataPtr->resize(stride * nVertices);
    auto vertexData = vertexDataPtr->data();
    ...
    mesh->setVertices(nVertices, vertexDataPtr);

    for (i = 0; i < nRings - 1; i++) {
        IndexData indexData;
        indexData.resize((nSlices + 1) * 2);
        for (int j = 0; j <= nSlices; j++) {
            indexData[j * 2 + 0] = i * (nSlices + 1) + j;
            indexData[j * 2 + 1] = (i + 1) * (nSlices + 1) + j;
        }
        mesh->addGroup(Mesh::TriStrip, ~0u, indexData);
    }
    return mesh;
}
