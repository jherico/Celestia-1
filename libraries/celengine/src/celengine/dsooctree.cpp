...
template <>
DynamicOctree<DeepSkyObject, double>::Pointer DynamicOctree<DeepSkyObject, double>::getChild(const DeepSkyObjectPtr& _obj, const PointType& cellCenterPos) {
...
template <>
uint32_t DynamicOctree<DeepSkyObject, double>::SPLIT_THRESHOLD = 10;
...
#ifdef CELVEC
    return abs(dsoPos.x - cellCenterPos.x) < dsoRadius || abs(dsoPos.y - cellCenterPos.y) < dsoRadius ||
           abs(dsoPos.z - cellCenterPos.z) < dsoRadius;
#endif
};

template <>
DynamicOctree<DeepSkyObject, double>::ExclusionFactorDecayFunction DynamicOctree<DeepSkyObject, double>::decayFunction =
    [](const double excludingFactor) -> double { return excludingFactor + 0.5f; };
...
#ifdef CELVEC
        double r = scale * (abs(plane->normal.x) + abs(plane->normal.y) + abs(plane->normal.z));
...
template <>
void StaticOctree<DeepSkyObject, double>::processCloseObjects(DSOHandler& processor,
                                    const PointType& obsPosition,
