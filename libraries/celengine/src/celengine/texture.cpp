...
static bool testMaxLevel() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, 8, 8, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, texels);
...
static void SetBorderColor(Color borderColor, GLenum target) {
    float bc[4] = { borderColor.red(), borderColor.green(), borderColor.blue(), borderColor.alpha() };
...
static void LoadMipmapSet(Image& img, GLenum target) {
    ...
    for (int mip = 0; mip < img.getMipLevelCount(); mip++) {
        uint32_t mipWidth = max((uint32_t)img.getWidth() >> mip, 1u);
        uint32_t mipHeight = max((uint32_t)img.getHeight() >> mip, 1u);

        if (img.isCompressed()) {
            glCompressedTexImage2DARB(target, mip, internalFormat, mipWidth, mipHeight, 0, img.getMipLevelSize(mip),
                                      img.getMipLevel(mip));
        } else {
            glTexImage2D(target, mip, internalFormat, mipWidth, mipHeight, 0, (GLenum)img.getFormat(), GL_UNSIGNED_BYTE,
                         img.getMipLevel(mip));
        }
    }
}
...
static void LoadMiplessTexture(Image& img, GLenum target) {
    ...
    if (img.isCompressed()) {
        glCompressedTexImage2DARB(target, 0, internalFormat, img.getWidth(), img.getHeight(), 0, img.getMipLevelSize(0),
                                  img.getMipLevel(0));
    } else {
        glTexImage2D(target, 0, internalFormat, img.getWidth(), img.getHeight(), 0, (GLenum)img.getFormat(), GL_UNSIGNED_BYTE,
                     img.getMipLevel(0));
    }
}
...
Texture::Texture(int w, int h, int d) : alpha(false), compressed(false), width(w), height(h), depth(d), formatOptions(0) {
}
...
uint32_t Texture::getFormatOptions() const {
    return formatOptions;
}

void Texture::setFormatOptions(uint32_t opts) {
    formatOptions = opts;
}

ImageTexture::ImageTexture(Image& img, AddressMode addressMode, MipMapMode mipMapMode) :
    Texture(img.getWidth(), img.getHeight()), glName(0) {
    ...
    if (mipmap) {
        if (precomputedMipMaps) {
            ...
            gluBuild2DMipmaps(GL_TEXTURE_2D, internalFormat, getWidth(), getHeight(), (GLenum)img.getFormat(), GL_UNSIGNED_BYTE,
                              img.getPixels());
        } else {
...
uint32_t ImageTexture::getName() const {
    return glName;
}
...
TiledTexture::TiledTexture(Image& img, int _uSplit, int _vSplit, MipMapMode mipMapMode) :
    Texture(img.getWidth(), img.getHeight()), uSplit(_uSplit), vSplit(_vSplit), glNames(NULL) {
    glNames = new uint32_t[uSplit * vSplit];
    {
    ...
    Image* tile = new Image(img.getFormat(), tileWidth, tileHeight, tileMipLevelCount);
    if (tile == NULL)
        return;
            ...
            if (precomputedMipMaps) {
                if (img.isCompressed()) {
                    for (int mip = 0; mip < tileMipLevelCount; mip++) {
                        ...
                        unsigned char* imgMip = img.getMipLevel(min(mip, mipLevelCount));
                        uint32_t mipWidth = max((uint32_t)img.getWidth() >> mip, 1u);
                        unsigned char* tileMip = tile->getMipLevel(mip);
                        uint32_t tileMipWidth = max((uint32_t)tile->getWidth() >> mip, 1u);
                        uint32_t tileMipHeight = max((uint32_t)tile->getHeight() >> mip, 1u);
                        ...
                        for (int y = 0; y < vBlocks; y++) {
                            memcpy(tileMip + y * destBytesPerRow, imgMip + tileOffset + y * srcBytesPerRow, destBytesPerRow);
                        }
                    }
                } else {
                ...
                if (img.isCompressed()) {
                    ...
                    for (int y = 0; y < vBlocks; y++) {
                        memcpy(tile->getPixels() + y * destBytesPerRow, img.getPixels() + tileOffset + y * srcBytesPerRow,
                               destBytesPerRow);
                    }
                } else {
                    unsigned char* tilePixels =
                        img.getPixels() + (v * tileHeight * img.getWidth() + u * tileWidth) * components;
                    for (int y = 0; y < tileHeight; y++) {
                        memcpy(tile->getPixels() + y * tileWidth * components, tilePixels + y * img.getWidth() * components,
                               tileWidth * components);
                    }
                }

                if (mipmap) {
                    gluBuild2DMipmaps(GL_TEXTURE_2D, internalFormat, tileWidth, tileHeight, (GLenum)tile->getFormat(),
                                      GL_UNSIGNED_BYTE, tile->getPixels());
                } else {
...
CubeMap::CubeMap(Image* faces[]) : Texture(faces[0]->getWidth(), faces[0]->getHeight()), glName(0) {
    // Verify that all the faces are square and have the same size
    int width = faces[0]->getWidth();
    int format = faces[0]->getFormat();
    int i = 0;
    for (i = 0; i < 6; i++) {
        if (faces[i]->getWidth() != width || faces[i]->getHeight() != width || faces[i]->getFormat() != format)
            return;
    }
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (mipmap) {
            if (precomputedMipMaps) {
                ...
                gluBuild2DMipmaps(targetFace, internalFormat, getWidth(), getHeight(), (GLenum)face->getFormat(),
                                  GL_UNSIGNED_BYTE, face->getPixels());
            }
        } else {
...
Texture* CreateProceduralTexture(int width,
                                 int height,
                                 int format,
...
extern Texture* CreateProceduralCubeMap(int size, int format, ProceduralTexEval func) {
    Image* faces[6];
    bool failed = false;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int y = 0; y < size; y++) {
                for (int x = 0; x < size; x++) {
                    ...
                    func(v.x(), v.y(), v.z(), face->getPixelRow(y) + x * face->getComponents());
                }
            }
        }
    }
...
static Texture* CreateTextureFromImage(Image& img, Texture::AddressMode addressMode, Texture::MipMapMode mipMode) {
    ...
    if ((img.getWidth() > maxDim || img.getHeight() > maxDim) && splittingAllowed) {
...
Texture::Pointer LoadTextureFromFile(const string& filename, Texture::AddressMode addressMode, Texture::MipMapMode mipMode) {
...
Texture::Pointer LoadHeightMapFromFile(const string& filename, float height, Texture::AddressMode addressMode) {
    Image* img = LoadImageFromFile(filename);
    if (img == NULL)
        return NULL;
    Image* normalMap = img->computeNormalMap(height, addressMode == Texture::Wrap);
    delete img;
    if (normalMap == NULL)
        return NULL;

    Texture* tex = CreateTextureFromImage(*normalMap, addressMode, Texture::DefaultMipMaps);
    delete normalMap;
