...
/** Create a new ModelGeometry wrapping the specified model.
  ...
  */
ModelGeometry::ModelGeometry(Model::Pointer model) : m_model(model) {
}

ModelGeometry::~ModelGeometry() {
}

bool ModelGeometry::pick(const Ray3d& r, double& distance) const {
 ...
 */
//void ModelGeometry::render(RenderContext& rc, double /* t */) {
...
//                    glBufferDataARB(GL_ARRAY_BUFFER_ARB, mesh->getVertexCount() * vertexDesc.stride, mesh->getVertexData(),
//                                    GL_STATIC_DRAW_ARB);
...
//    uint32_t materialCount = m_model->getMaterialCount();
//
...
//        for (uint32_t groupIndex = 0; groupIndex < mesh->getGroupCount(); ++groupIndex) {
//            const Mesh::PrimitiveGroup* group = mesh->getGroup(groupIndex);
//
//            // Set up the material
//            const Material* material = NULL;
//            uint32_t materialIndex = group->materialIndex;
...
bool ModelGeometry::isOpaque() const {
    return m_model->isOpaque();
}

bool ModelGeometry::isNormalized() const {
    return m_model->isNormalized();
}

bool ModelGeometry::usesTextureType(Material::TextureSemantic t) const {
    return m_model->usesTextureType(t);
}

void ModelGeometry::loadTextures() {
#if 0
...
string CelestiaTextureResource::source() const {
