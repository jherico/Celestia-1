...
ParticleSystemLoader::ParticleSystemLoader(istream& in) : m_tokenizer(&in), m_parser(&m_tokenizer) {
}

ParticleSystemLoader::~ParticleSystemLoader() {
}

ParticleSystemPtr ParticleSystemLoader::load() {
    ...
    while (m_tokenizer.nextToken() != Tokenizer::TokenEnd) {
        ...
        auto objParamsValue = m_parser.readValue();
        ...
        if (objType == "Emitter") {
            auto emitter = parseEmitter(objParams);
            if (emitter == NULL) {
                return NULL;
            }
...
VectorGeneratorPtr ParticleSystemLoader::parseGenerator(const HashPtr& params) {
    Vector3f constantValue(0.0f, 0.0f, 0.0f);
    if (params->getVector("Constant", constantValue)) {
        return std::make_shared<ConstantGenerator>(constantValue);
    }

    auto generatorValue = params->getValue("Box");
    if (generatorValue != NULL) {
        ...
        const HashPtr& boxParams = generatorValue->getHash();
        ...
        return std::make_shared<BoxGenerator>(center, size);
    }
    ...
    if (generatorValue != NULL) {
        ...
        const HashPtr& lineParams = generatorValue->getHash();
        ...
        return std::make_shared<LineGenerator>(p0, p1);
    }
    ...
    if (generatorValue != NULL) {
        ...
        const HashPtr& ellipsoidSurfaceParams = generatorValue->getHash();
        ...
        return std::make_shared<EllipsoidSurfaceGenerator>(center, size * 0.5f);
    }
    ...
    if (generatorValue != NULL) {
        ...
        const HashPtr& coneParams = generatorValue->getHash();
        ...
        return std::make_shared<ConeGenerator>((float)degToRad(minAngle), (float)degToRad(maxAngle), (float)minSpeed, (float)maxSpeed);
    }
    ...
    if (generatorValue != NULL) {
        ...
        const HashPtr& gaussianDiscParams = generatorValue->getHash();

        double sigma = 1.0;
        gaussianDiscParams->getNumber("Sigma", sigma);

        return std::make_shared<GaussianDiscGenerator>((float)sigma);
    }
...
ParticleEmitter::Pointer ParticleSystemLoader::parseEmitter(const HashPtr& params) {
    string textureFileName;
    ResourceHandle textureHandle = InvalidResource;
    if (params->getString("Texture", textureFileName)) {
        textureHandle =
            GetTextureManager()->getHandle(TextureInfo(textureFileName, getTexturePath(), TextureInfo::BorderClamp));
    }
    ...
    VectorGeneratorPtr initialPositionGenerator = NULL;
    auto positionValue = params->getValue("InitialPosition");
    if (positionValue == NULL) {
        initialPositionGenerator = std::make_shared<ConstantGenerator>(Vector3f::Zero());
    } else {
    ...
    VectorGeneratorPtr initialVelocityGenerator = NULL;
    auto velocityValue = params->getValue("InitialVelocity");
    if (velocityValue == NULL) {
        initialVelocityGenerator = std::make_shared<ConstantGenerator>(Vector3f::Zero());
    } else {
...
void ParticleSystemLoader::raiseError(const string& errorMessage) {
    m_errorMessage = errorMessage;
}

const string& ParticleSystemLoader::getErrorMessage() const {
    return m_errorMessage;
}

void ParticleSystemLoader::setTexturePath(const string& texPath) {
    m_texPath = texPath;
}

const string& ParticleSystemLoader::getTexturePath() const {
    return m_texPath;
}

ParticleSystemPtr LoadParticleSystem(istream& in, const string& texPath) {
    ParticleSystemLoader loader(in);
    loader.setTexturePath(texPath);

    auto particleSystem = loader.load();
