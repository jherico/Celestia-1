...
static const int HOUR_MIN_SEC_SPACING[] = { 2 * HR,  1 * HR,  30 * MIN,   15 * MIN,   10 * MIN,  5 * MIN, 3 * MIN,
                                            2 * MIN, 1 * MIN, 30 * SEC,   15 * SEC,   10 * SEC,  5 * SEC, 3 * SEC,
                                            2 * SEC, 1 * SEC, 500 * MSEC, 200 * MSEC, 100 * MSEC };

static const int DEG_MIN_SEC_SPACING[] = { 30 * DEG, 15 * DEG, 10 * DEG, 5 * DEG, 3 * DEG, 2 * DEG,    1 * DEG,    30 * MIN,
                                           15 * MIN, 10 * MIN, 5 * MIN,  3 * MIN, 2 * MIN, 1 * MIN,    30 * SEC,   15 * SEC,
                                           10 * SEC, 5 * SEC,  3 * SEC,  2 * SEC, 1 * SEC, 500 * MSEC, 200 * MSEC, 100 * MSEC };
...
SkyGrid::SkyGrid() :
    m_orientation(Quaterniond::Identity()), m_lineColor(Color::White), m_labelColor(Color::White),
    m_longitudeUnits(LongitudeHours), m_longitudeDirection(IncreasingCounterclockwise) {
}
...
static Renderer::LabelAlignment getCoordLabelHAlign(int planeIndex) {
    switch (planeIndex) {
        case 2:
...
static Renderer::LabelVerticalAlignment getCoordLabelVAlign(int planeIndex) {
    if (planeIndex == 1)
...
template <typename T>
static bool planeCircleIntersection(const Matrix<T, 3, 1>& planeNormal,
...
string SkyGrid::latitudeLabel(int latitude, int latitudeStep) const {
...
string SkyGrid::longitudeLabel(int longitude, int longitudeStep) const {
...
int SkyGrid::parallelSpacing(double idealSpacing) const {
...
int SkyGrid::meridianSpacing(double idealSpacing) const {
