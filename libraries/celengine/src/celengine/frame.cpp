...
ReferenceFrame::ReferenceFrame(Selection center) : centerObject(center), refCount(0) {
}
 ...
 */
UniversalCoord ReferenceFrame::convertFromUniversal(const UniversalCoord& uc, double tjd) const {
    UniversalCoord uc1 = uc.difference(centerObject.getPosition(tjd));
    return rotate(uc1, getOrientation(tjd).conjugate());
}

Quaterniond ReferenceFrame::convertFromUniversal(const Quaterniond& q, double tjd) const {
 ...
 */
UniversalCoord ReferenceFrame::convertToUniversal(const UniversalCoord& uc, double tjd) const {
    return centerObject.getPosition(tjd) + rotate(uc, getOrientation(tjd));
}

Quaterniond ReferenceFrame::convertToUniversal(const Quaterniond& q, double tjd) const {
    return q * getOrientation(tjd);
}

Vector3d ReferenceFrame::convertFromAstrocentric(const Vector3d& p, double tjd) const {
...
Vector3d ReferenceFrame::convertToAstrocentric(const Vector3d& p, double tjd) const {
    if (centerObject.getType() == Selection::Type_Body) {
        Vector3d center = centerObject.body()->getAstrocentricPosition(tjd);
 ...
 */
Selection ReferenceFrame::getCenter() const {
    return centerObject;
}

Vector3d ReferenceFrame::getAngularVelocity(double tjd) const {
...
uint32_t ReferenceFrame::nestingDepth(uint32_t maxDepth, FrameType frameType) const {
    return this->nestingDepth(0, maxDepth, frameType);
}

static uint32_t getFrameDepth(const Selection& sel, uint32_t depth, uint32_t maxDepth, ReferenceFrame::FrameType frameType) {
    if (depth > maxDepth)
        return depth;
    ...
    uint32_t orbitFrameDepth = depth;
    uint32_t bodyFrameDepth = depth;
...
bool J2000EclipticFrame::isInertial() const {
    return true;
}

uint32_t J2000EclipticFrame::nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType) const {
...
Quaterniond J2000EquatorFrame::getOrientation(double /* tjd */) const {
    return Quaterniond(AngleAxis<double>(astro::J2000Obliquity, Vector3d::UnitX()));
}

bool J2000EquatorFrame::isInertial() const {
    return true;
}

uint32_t J2000EquatorFrame::nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType) const {
    return getFrameDepth(getCenter(), depth, maxDepth, PositionFrame);
}

/*** BodyFixedFrame ***/

BodyFixedFrame::BodyFixedFrame(Selection center, Selection obj) : ReferenceFrame(center), fixObject(obj) {
}

Quaterniond BodyFixedFrame::getOrientation(double tjd) const {
...
Vector3d BodyFixedFrame::getAngularVelocity(double tjd) const {
    switch (fixObject.getType()) {
        case Selection::Type_Body:
...
bool BodyFixedFrame::isInertial() const {
    return false;
}

uint32_t BodyFixedFrame::nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType) const {
    uint32_t n = getFrameDepth(getCenter(), depth, maxDepth, PositionFrame);
    if (n > maxDepth) {
        return n;
    } else {
        uint32_t m = getFrameDepth(fixObject, depth, maxDepth, OrientationFrame);
        return max(m, n);
    }
}
...
Quaterniond BodyMeanEquatorFrame::getOrientation(double tjd) const {
...
Vector3d BodyMeanEquatorFrame::getAngularVelocity(double tjd) const {
    if (isFrozen) {
        return Vector3d::Zero();
    } else {
...
bool BodyMeanEquatorFrame::isInertial() const {
    if (isFrozen) {
        return true;
    } else {
...
uint32_t BodyMeanEquatorFrame::nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType) const {
    // Test origin and equator object (typically the same) frames
    uint32_t n = getFrameDepth(getCenter(), depth, maxDepth, PositionFrame);
    if (n > maxDepth) {
        return n;
    } else {
        uint32_t m = getFrameDepth(equatorObject, depth, maxDepth, OrientationFrame);
        return max(m, n);
    }
}
...
CachingFrame::CachingFrame(Selection _center) :
    ReferenceFrame(_center), lastTime(-1.0e50), lastOrientation(Quaterniond::Identity()), lastAngularVelocity(0.0, 0.0, 0.0),
    orientationCacheValid(false), angularVelocityCacheValid(false) {
}

Quaterniond CachingFrame::getOrientation(double tjd) const {
    if (tjd != lastTime) {
        lastTime = tjd;
...
TwoVectorFrame::TwoVectorFrame(Selection center, const FrameVector& prim, int primAxis, const FrameVector& sec, int secAxis) :
    CachingFrame(center), primaryVector(prim), primaryAxis(primAxis), secondaryVector(sec), secondaryAxis(secAxis) {
...
Quaterniond TwoVectorFrame::computeOrientation(double tjd) const {
...
bool TwoVectorFrame::isInertial() const {
...
uint32_t TwoVectorFrame::nestingDepth(uint32_t depth, uint32_t maxDepth, FrameType) const {
    ...
    uint32_t m = primaryVector.nestingDepth(depth, maxDepth);
    n = max(m, n);
    if (n > maxDepth)
        return n;
...
FrameVector& FrameVector::operator=(const FrameVector& fv) {
    vecType = fv.vecType;
...
FrameVector::FrameVector(FrameVectorType t) : vecType(t), observer(), target(), vec(0.0, 0.0, 0.0), frame(NULL) {
}

FrameVector::~FrameVector() {
}

FrameVector FrameVector::createRelativePositionVector(const Selection& _observer, const Selection& _target) {
...
FrameVector FrameVector::createRelativeVelocityVector(const Selection& _observer, const Selection& _target) {
...
FrameVector FrameVector::createConstantVector(const Vector3d& _vec, const ReferenceFrameConstPtr& _frame) {
...
Vector3d FrameVector::direction(double tjd) const {
    Vector3d v;
...
uint32_t FrameVector::nestingDepth(uint32_t depth, uint32_t maxDepth) const {
    switch (vecType) {
        case RelativePosition:
        case RelativeVelocity: {
            uint32_t n = getFrameDepth(observer, depth, maxDepth, ReferenceFrame::PositionFrame);
            if (n > maxDepth) {
                return n;
            } else {
                uint32_t m = getFrameDepth(target, depth, maxDepth, ReferenceFrame::PositionFrame);
                return max(m, n);
            }
        } break;
