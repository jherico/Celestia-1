...
class ObserverFrame {
public:
    using Pointer = std::shared_ptr<ObserverFrame>;
    ...
    ObserverFrame();
    ObserverFrame(CoordinateSystem cs, const Selection& _refObject, const Selection& _targetObj = Selection());
    ObserverFrame(const ObserverFrame&);
    ObserverFrame(const ReferenceFramePtr& f);

    ~ObserverFrame();

    ObserverFrame& operator=(const ObserverFrame& f);

    CoordinateSystem getCoordinateSystem() const { return coordSys; }
    Selection getRefObject() const { return frame->getCenter(); }
    Selection getTargetObject() const { return targetObject; }

    const ReferenceFrameConstPtr& getFrame() const { return frame; }
    ...
    static UniversalCoord convert(const ObserverFramePtr& fromFrame,
                                  const ObserverFramePtr& toFrame,
                                  const UniversalCoord& uc,
                                  double t);
    static Eigen::Quaterniond convert(const ObserverFramePtr& fromFrame,
                                      const ObserverFramePtr& toFrame,
                                      const Eigen::Quaterniond& q,
                                      double t);

private:
    static ReferenceFrameConstPtr createFrame(CoordinateSystem _coordSys,
                                         const Selection& _refObject,
                                         const Selection& _targetObject);

private:
    CoordinateSystem coordSys;
    ReferenceFrameConstPtr frame;
...
public:
    ...
    */
    ...
    const Selection& getTrackedObject() const { return trackObject; }
    void setTrackedObject(const Selection& sel) { trackObject = sel; }

    const std::string& getDisplayedSurface() const { return displayedSurface; }
    void setDisplayedSurface(const std::string& surf) { displayedSurface = surf; }

    uint32_t getLocationFilter() const { return locationFilter; }
    void setLocationFilter(uint32_t _locationFilter) { locationFilter = _locationFilter; }

    void gotoSelection(const Selection&, double gotoTime, const Eigen::Vector3f& up, ObserverFrame::CoordinateSystem upFrame);
    ...
    ObserverMode getMode() const { return observerMode; }
    void setMode(ObserverMode mode) { observerMode = mode; }
...
private:
    ...
    void convertFrameCoordinates(const ObserverFramePtr& newFrame);
