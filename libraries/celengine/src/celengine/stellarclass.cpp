...
// large enough.)
char* StellarClass::str(char* buf, uint32_t buflen) const {
    ...
    if (st == StellarClass::WhiteDwarf) {
        ...
        s0[0] = "OBAFGKMRSNWW?LTC"[(uint32_t)getSpectralClass()];
        s0[1] = '\0';
...
uint16_t StellarClass::pack() const {
    return (((uint16_t)starType << 12) | (((uint16_t)specClass & 0xf) << 8) | ((uint16_t)subclass << 4) | ((uint16_t)lumClass));
}

bool StellarClass::unpack(uint16_t st) {
...
StellarClass StellarClass::parse(const string& st) {
    uint32_t i = 0;
    ...
    uint32_t subclass = StellarClass::Subclass_Unknown;
    ...
    while (state != EndState) {
        ...
        switch (state) {
            ...
            case NormalStarSubclassState:
                if (isdigit(c)) {
                    subclass = (uint32_t)c - (uint32_t)'0';
