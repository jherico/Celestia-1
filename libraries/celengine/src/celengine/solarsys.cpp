...
static const int CLASSES_UNCLICKABLE = Body::Invisible | Body::Diffuse;

static const int CLASSES_INVISIBLE_AS_POINT = Body::Invisible | Body::SurfaceFeature | Body::Component | Body::Diffuse;

static const int CLASSES_SECONDARY_ILLUMINATOR =
    Body::Planet | Body::Moon | Body::MinorMoon | Body::DwarfPlanet | Body::Asteroid | Body::Comet;
...
static uint32_t MaxFrameDepth = 50;
...
static LocationPtr CreateLocation(const HashPtr& locationData, const BodyPtr& body) {
    LocationPtr location = std::make_shared<Location>();
...
static void FillinSurface(const HashPtr& surfaceData, Surface& surface, const std::string& path) {
    ...
    if (surfaceData->getColor("HazeColor", hazeColor) | surfaceData->getNumber("HazeDensity", hazeDensity)) {
        surface.hazeColor = Color(hazeColor.red(), hazeColor.green(), hazeColor.blue(), hazeDensity);
    }
    ...
    bool separateSpecular = surfaceData->getString("SpecularTexture", specularTexture);
    bool applyNormalMap = surfaceData->getString("NormalMap", normalTexture);
    bool applyOverlay = surfaceData->getString("OverlayTexture", overlayTexture);

    uint32_t baseFlags = TextureInfo::WrapTexture | TextureInfo::AllowSplitting;
    uint32_t bumpFlags = TextureInfo::WrapTexture | TextureInfo::AllowSplitting;
    uint32_t nightFlags = TextureInfo::WrapTexture | TextureInfo::AllowSplitting;
    uint32_t specularFlags = TextureInfo::WrapTexture | TextureInfo::AllowSplitting;
...
static Selection GetParentObject(const PlanetarySystemPtr& system) {
    Selection parent;
    const auto& primary = system->getPrimaryBody();
    if (primary != NULL)
...
TimelinePhasePtr CreateTimelinePhase(const BodyPtr& body,
                                     Universe& universe,
                                     const HashPtr& phaseData,
                                     const string& path,
                                     const ReferenceFramePtr& defaultOrbitFrame,
                                     const ReferenceFramePtr& defaultBodyFrame,
                                     bool isFirstPhase,
                                     bool isLastPhase,
    ...
    ReferenceFramePtr orbitFrame;
    auto frameValue = phaseData->getValue("OrbitFrame");
    if (frameValue != NULL) {
    ...
    auto bodyFrameValue = phaseData->getValue("BodyFrame");
    ...
    if (!rotationModel) {
        ...
        rotationModel = std::make_shared<ConstantOrientation>(Quaterniond::Identity());
    }

    auto phase =
        TimelinePhase::CreateTimelinePhase(universe, body, beginning, ending, orbitFrame, orbit, bodyFrame, rotationModel);

    // Frame ownership transfered to phase; release local references
    return phase;
}

TimelinePtr CreateTimelineFromArray(const BodyPtr& body,
                                    Universe& universe,
                                    const ValueArrayPtr& timelineArray,
                                    const string& path,
                                    const ReferenceFramePtr& defaultOrbitFrame,
                                    const ReferenceFramePtr& defaultBodyFrame) {
    ...
    for (auto iter = timelineArray->begin(); iter != timelineArray->end(); iter++) {
        const auto& phaseData = (*iter)->getHash();
        if (phaseData == NULL) {
            clog << "Error in timeline of '" << body->getName() << "': phase " << iter - timelineArray->begin() + 1
                 << " is not a property group.\n";
            return NULL;
        }
        ...
        auto phase = CreateTimelinePhase(body, universe, phaseData, path, defaultOrbitFrame, defaultBodyFrame, isFirstPhase,
                                         isLastPhase, previousEnding);
        if (phase == NULL) {
...
static bool CreateTimeline(const BodyPtr& body,
                           const PlanetarySystemPtr& system,
                           Universe& universe,
                           const HashPtr& planetData,
                           const string& path,
                           Disposition disposition,
                           BodyType bodyType) {
    FrameTreePtr parentFrameTree = NULL;
    ...
    if (parentObject.body()) {
        ...
        auto solarSystem = universe.getSolarSystem(parentObject.star());
        if (solarSystem == NULL)
    ...
    ReferenceFramePtr defaultOrbitFrame = NULL;
    ReferenceFramePtr defaultBodyFrame = NULL;
    ...
    // things the old way.
    auto value = planetData->getValue("Timeline");
    if (value != NULL) {
        ...
        auto timeline = CreateTimelineFromArray(body, universe, value->getArray(), path, defaultOrbitFrame, defaultBodyFrame);
        if (timeline == NULL) {
            return false;
        } else {
    ...
    ReferenceFramePtr orbitFrame = NULL;
    ReferenceFramePtr bodyFrame = NULL;
    OrbitPtr orbit = NULL;
    RotationModelPtr rotationModel = NULL;
    ...
    if (disposition == ModifyObject) {
        auto timeline = body->getTimeline();
    ...
    auto frameValue = planetData->getValue("OrbitFrame");
    if (frameValue != NULL) {
    ...
    auto bodyFrameValue = planetData->getValue("BodyFrame");
    if (bodyFrameValue != NULL) {
...
static BodyPtr CreateBody(const string& name,
                          const PlanetarySystemPtr system,
                          Universe& universe,
                          const BodyPtr& existingBody,
                          const HashPtr& planetData,
                          const string& path,
                          Disposition disposition,
                          BodyType bodyType) {
    ...
    if (body == NULL) {
        body = std::make_shared<Body>(system, name);
        system->addBody(body);
        ...
        if (atmosDataValue != NULL) {
            if (atmosDataValue->getType() != Value::HashType) {
                ...
                if (atmosData->getString("CloudNormalMap", cloudNormalMap)) {
                    atmosphere->cloudNormalMap.setTexture(cloudNormalMap, path, TextureInfo::WrapTexture);
                }
...
static BodyPtr CreateReferencePoint(const string& name,
                                    const PlanetarySystemPtr& system,
                                    Universe& universe,
                                    const BodyPtr& existingBody,
                                    const HashPtr& refPointData,
                                    const string& path,
                                    Disposition disposition) {
    BodyPtr body = NULL;
...
bool LoadSolarSystemObjects(istream& in, Universe& universe, const std::string& directory) {
    ...
    while (tokenizer.nextToken() != Tokenizer::TokenEnd) {
        ...
        auto objectDataValue = parser.readValue();
        ...
        const auto& objectData = objectDataValue->getHash();

        Selection parent = universe.findPath(parentName, NULL, 0);
        PlanetarySystemPtr parentSystem = NULL;
        ...
        if (bodyType != UnknownBodyType) {
            //bool orbitsPlanet = false;
            if (parent.star() != NULL) {
                auto solarSystem = universe.getSolarSystem(parent.star());
                if (solarSystem == NULL) {
                ...
                if (parentSystem == NULL) {
                    ...
                    parentSystem = std::make_shared<PlanetarySystem>(parent.body());
            ...
            if (parentSystem != NULL) {
                auto existingBody = parentSystem->find(primaryName);
                if (existingBody) {
                    if (disposition == AddObject) {
                        errorMessagePrelude(tokenizer);
                        cerr << _("warning duplicate definition of ") << parentName << " " << primaryName << '\n';
                ...
                if (bodyType == ReferencePoint)
                    body = CreateReferencePoint(primaryName, parentSystem, universe, existingBody, objectData, directory,
                                                disposition);
                else
                    body = CreateBody(primaryName, parentSystem, universe, existingBody, objectData, directory, disposition,
                                      bodyType);
            ...
            if (parent.body() != NULL) {
                auto location = CreateLocation(objectData, parent.body());
                if (location != NULL) {
...
SolarSystem::SolarSystem(const StarPtr& _star) : star(_star), planets(NULL), frameTree(NULL) {
    planets = std::make_shared<PlanetarySystem>(star);
    frameTree = std::make_shared<FrameTree>(star);
}
