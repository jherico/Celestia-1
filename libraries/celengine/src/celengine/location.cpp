...
Location::Location() :
    parent(NULL), position(Vector3f::Zero()), size(0.0f), importance(-1.0f), featureType(Other), overrideLabelColor(false),
    labelColor(1.0f, 1.0f, 1.0f), infoURL(NULL) {
}
...
const BodyPtr& Location::getParentBody() const {
    return parent;
}

void Location::setParentBody(const BodyPtr& _parent) {
    parent = _parent;
}
