...
MultiResTexture::MultiResTexture(ResourceHandle loTex, ResourceHandle medTex, ResourceHandle hiTex) {
    tex[lores] = loTex;
    tex[medres] = medTex;
    tex[hires] = hiTex;
}

MultiResTexture::MultiResTexture(const string& source, const string& path) {
    setTexture(source, path);
}

void MultiResTexture::setTexture(const string& source, const string& path, uint32_t flags) {
...
void MultiResTexture::setTexture(const string& source, const string& path, float bumpHeight, uint32_t flags) {
    TextureManager* texMan = GetTextureManager();
...
Texture::Pointer MultiResTexture::find(uint32_t resolution) {
    TextureManager* texMan = GetTextureManager();

    auto res = texMan->find(tex[resolution]);
    if (res != NULL)
        return res;
    ...
    uint32_t secondChoice = medres;
    uint32_t lastResort = hires;
    switch (resolution) {
        case lores:
            secondChoice = medres;
            lastResort = hires;
...
bool MultiResTexture::isValid() const {
    return (tex[lores] != InvalidResource || tex[medres] != InvalidResource || tex[hires] != InvalidResource);
}
