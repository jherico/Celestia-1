...
using ScriptedRotation = RotationModel;
using ScriptedOrbit = Orbit;
 ...
 */
static void GetDefaultUnits(bool usePlanetUnits, double& distanceScale, double& timeScale) {
    if (usePlanetUnits) {
 ...
 * @param[out] distanceScale The default distance scale in kilometers.
 */
static void GetDefaultUnits(bool usePlanetUnits, double& distanceScale) {
    distanceScale = (usePlanetUnits) ? KM_PER_AU : 1.0;
}

bool ParseDate(const HashPtr& hash, const string& name, double& jd) {
 ...
 */
static std::shared_ptr<EllipticalOrbit> CreateEllipticalOrbit(const HashPtr& orbitData, bool usePlanetUnits) {
    ...
    return std::make_shared<EllipticalOrbit>(pericenterDistance, eccentricity, degToRad(inclination), degToRad(ascendingNode),
                               degToRad(argOfPericenter), degToRad(anomalyAtEpoch), period, epoch);
}
 ...
 */
static Orbit::Pointer CreateSampledTrajectory(const HashPtr& trajData, const string& path) {
    string sourceName;
    ...
    auto orbit = GetTrajectoryManager()->find(orbitHandle);
    if (orbit == NULL) {
 ...
 * is BodyFixed.
 */
static Orbit::Pointer CreateFixedPosition(const HashPtr& trajData, const Selection& centralObject, bool usePlanetUnits) {
    double distanceScale;
    ...
    return std::make_shared<FixedOrbit>(position);
}

/**
 * Parse a string list--either a single string or an array of strings is permitted.
 */
static bool ParseStringList(const HashPtr& table, const string& propertyName, list<string>& stringList) {
    auto v = table->getValue(propertyName);
    if (v == NULL)
        return NULL;
 ...
 *  used.
 */
static SpiceOrbit::Pointer CreateSpiceOrbit(const HashPtr& orbitData, const string& path, bool usePlanetUnits) {
    ...
    auto beginningDate = orbitData->getValue("Beginning");
    auto endingDate = orbitData->getValue("Ending");
    ...
    if (beginningDate != NULL && endingDate != NULL) {
        ...
        orbit = std::make_shared<SpiceOrbit>(targetBodyName, originName, period, boundingRadius, beginningTDBJD, endingTDBJD);
    } else {
        // No time interval given; we'll use whatever coverage window is given
        // in the SPICE kernel.
        orbit = std::make_shared<SpiceOrbit>(targetBodyName, originName, period, boundingRadius);
    }
 ...
 */
static SpiceRotation::Pointer CreateSpiceRotation(const HashPtr& rotationData, const string& path) {
    string frameName;
    ...
    // Either a complete time interval must be specified with Beginning/Ending, or
    // else neither field can be present.
    auto beginningDate = rotationData->getValue("Beginning");
    auto endingDate = rotationData->getValue("Ending");
    if (beginningDate != NULL && endingDate == NULL) {
    ...
    SpiceRotation::Pointer rotation = NULL;
    if (beginningDate != NULL && endingDate != NULL) {
        double beginningTDBJD = 0.0;
        ...
        rotation = std::make_shared<SpiceRotation>(frameName, baseFrameName, period, beginningTDBJD, endingTDBJD);
    } else {
        // No time interval given; rotation is valid at any time.
        rotation = std::make_shared<SpiceRotation>(frameName, baseFrameName, period);
    }
...
static ScriptedOrbit::Pointer CreateScriptedOrbit(const HashPtr& orbitData, const string& path) {
#if !defined(CELX)
    ...
    auto pathValue = new Value(*pathCopy);
...
Orbit::Pointer CreateOrbit(const Selection& centralObject, const HashPtr& planetData, const string& path, bool usePlanetUnits) {
    Orbit::Pointer orbit;
...
#ifdef USE_SPICE
    auto spiceOrbitDataValue = planetData->getValue("SpiceOrbit");
    ...
    auto scriptedOrbitValue = planetData->getValue("ScriptedOrbit");
    ...
    auto sampledTrajDataValue = planetData->getValue("SampledTrajectory");
    ...
    if (planetData->getString("SampledOrbit", sampOrbitFile)) {
        ...
        ResourceHandle orbitHandle = GetTrajectoryManager()->getHandle(
            TrajectoryInfo(sampOrbitFile, path, TrajectoryInterpolationCubic, TrajectoryPrecisionSingle));
    ...
    auto orbitDataValue = planetData->getValue("EllipticalOrbit");
    if (orbitDataValue != NULL) {
        if (orbitDataValue->getType() != Value::HashType) {
            clog << "Object has incorrect elliptical orbit syntax.\n";
            return NULL;
        } else {
            return CreateEllipticalOrbit(orbitDataValue->getHash(), usePlanetUnits);
        }
    }
    ...
    auto fixedPositionValue = planetData->getValue("FixedPosition");
    if (fixedPositionValue != NULL) {
        ...
        if (planetData->getLengthVector("FixedPosition", fixedPosition, 1.0, distanceScale)) {
            // Convert to Celestia's coordinate system
            fixedPosition = Vector3d(fixedPosition.x(), fixedPosition.z(), -fixedPosition.y());

            return std::make_shared<FixedOrbit>(fixedPosition);
    ...
    if (planetData->getSphericalTuple("LongLat", longlat)) {
        auto centralBody = centralObject.body();
        if (centralBody != NULL) {
            Vector3d pos = centralBody->planetocentricToCartesian(longlat.x(), longlat.y(), longlat.z());
            return std::make_shared<SynchronousOrbit>(*centralBody, pos);
        } else {
...
static RotationModel::Pointer CreateFixedRotationModel(double offset, double inclination, double ascendingNode) {
    Quaterniond q = YRotation(-PI - offset) * XRotation(-inclination) * YRotation(-ascendingNode);

    return std::make_shared<ConstantOrientation>(q);
}

static RotationModel::Pointer CreateUniformRotationModel(const HashPtr& rotationData, double syncRotationPeriod) {
    ...
    if (period == 0.0) {
        ...
        return std::make_shared<UniformRotationModel>(period, offset, epoch, inclination, ascendingNode);
    }
}

static RotationModel::Pointer CreateFixedRotationModel(const HashPtr& rotationData) {
    double offset = 0.0;
    if (rotationData->getAngle("MeridianAngle", offset)) {
        offset = degToRad(offset);
    }
...
static RotationModel::Pointer CreateFixedAttitudeRotationModel(const HashPtr& rotationData) {
    double heading = 0.0;
    ...
    Quaterniond q = YRotation(-PI - heading) * XRotation(-tilt) * ZRotation(-roll);

    return std::make_shared<ConstantOrientation>(q);
}

static RotationModel::Pointer CreatePrecessingRotationModel(const HashPtr& rotationData, double syncRotationPeriod) {
    // Default to synchronous rotation
    double period = syncRotationPeriod;
    rotationData->getTime("Period", period, 1.0, 1.0 / HOURS_PER_DAY);
    ...
    if (period == 0.0) {
        ...
        return std::make_shared<PrecessingRotationModel>(period, offset, epoch, inclination, ascendingNode, precessionPeriod);
    }
}

static ScriptedRotation::Pointer CreateScriptedRotation(const HashPtr& rotationData, const string& path) {
#if !defined(CELX)
 ...
 */
RotationModel::Pointer CreateRotationModel(const HashPtr& planetData, const string& path, double syncRotationPeriod) {
    RotationModel::Pointer rotationModel = NULL;
    ...
    if (planetData->getString("CustomRotation", customRotationModelName)) {
        ...
        clog << "Could not find custom rotation model named '" << customRotationModelName << "'\n";
    }

#ifdef USE_SPICE
    auto spiceRotationDataValue = planetData->getValue("SpiceRotation");
    ...
    auto scriptedRotationValue = planetData->getValue("ScriptedRotation");
    ...
    if (planetData->getString("SampledOrientation", sampOrientationFile)) {
        ...
        ResourceHandle orientationHandle = GetRotationModelManager()->getHandle(RotationModelInfo(sampOrientationFile, path));
        rotationModel = GetRotationModelManager()->find(orientationHandle);
        if (rotationModel != NULL) {
            return rotationModel;
        }

        clog << "Could not load rotation model file '" << sampOrientationFile << "'\n";
    }

    auto precessingRotationValue = planetData->getValue("PrecessingRotation");
    if (precessingRotationValue != NULL) {
        if (precessingRotationValue->getType() != Value::HashType) {
            ...
            return CreatePrecessingRotationModel(precessingRotationValue->getHash(), syncRotationPeriod);
        }
    }

    auto uniformRotationValue = planetData->getValue("UniformRotation");
    if (uniformRotationValue != NULL) {
        if (uniformRotationValue->getType() != Value::HashType) {
            ...
            return CreateUniformRotationModel(uniformRotationValue->getHash(), syncRotationPeriod);
        }
    }

    auto fixedRotationValue = planetData->getValue("FixedRotation");
    ...
    auto fixedAttitudeValue = planetData->getValue("FixedAttitude");
    ...
    if (specified) {
        ...
        if (period == 0.0) {
            ...
            rm = std::make_shared<UniformRotationModel>(period, offset, epoch, inclination, ascendingNode);
        } else {
            rm = std::make_shared<PrecessingRotationModel>(period, offset, epoch, inclination, ascendingNode, -360.0 / precessionRate);
        }
...
RotationModelPtr CreateDefaultRotationModel(double syncRotationPeriod) {
    if (syncRotationPeriod == 0.0) {
        ...
        return std::make_shared<ConstantOrientation>(Quaterniond::Identity());
    } else {
        return std::make_shared<UniformRotationModel>(syncRotationPeriod, 0.0f, astro::J2000, 0.0f, 0.0f);
    }
}
 ...
 */
static Selection getFrameCenter(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
    string centerName;
...
static std::shared_ptr<BodyFixedFrame> CreateBodyFixedFrame(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
...
static ReferenceFramePtr CreateMeanEquatorFrame(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
    Selection center = getFrameCenter(universe, frameData, defaultCenter);
    if (center.empty())
        return NULL;
 ...
 */
static int parseAxisLabel(const std::string& label) {
...
static int getAxis(const HashPtr& vectorData) {
    string axisLabel;
 ...
 */
static Selection getVectorTarget(const Universe& universe, const HashPtr& vectorData) {
    string targetName;
 ...
 * empty selection if it's missing or refers to an object that doesn't exist.
 */
static Selection getVectorObserver(const Universe& universe, const HashPtr& vectorData) {
    string obsName;
...
static FrameVector::Pointer CreateFrameVector(const Universe& universe, const Selection& center, const HashPtr& vectorData) {
    auto value = vectorData->getValue("RelativePosition");
    if (value != NULL && value->getHash() != NULL) {
        const HashPtr& relPosData = value->getHash();
        ...
        if (observer.empty() || target.empty())
            ...
            return std::make_shared<FrameVector>(FrameVector::createRelativePositionVector(observer, target));
    }

    value = vectorData->getValue("RelativeVelocity");
    if (value != NULL && value->getHash() != NULL) {
        const HashPtr& relVData = value->getHash();
        ...
        if (observer.empty() || target.empty())
            ...
            return std::make_shared<FrameVector>(FrameVector::createRelativeVelocityVector(observer, target));
    }

    value = vectorData->getValue("ConstantVector");
    if (value != NULL && value->getHash() != NULL) {
        const HashPtr& constVecData = value->getHash();
        ...
        // J2000 ecliptic.
        ReferenceFrame::Pointer f;
        auto frameValue = constVecData->getValue("Frame");
        if (frameValue != NULL) {
        ...
        return std::make_shared<FrameVector>(FrameVector::createConstantVector(vec, f));
    } else {
...
static ReferenceFrame::Pointer CreateTwoVectorFrame(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
    Selection center = getFrameCenter(universe, frameData, defaultCenter);
    if (center.empty())
        return NULL;

    // Primary and secondary vector definitions are required
    auto primaryValue = frameData->getValue("Primary");
    if (!primaryValue) {
        clog << "Primary axis missing from two-vector frame.\n";
        return NULL;
    }

    const HashPtr& primaryData = primaryValue->getHash();
    if (!primaryData) {
        clog << "Bad syntax for primary axis of two-vector frame.\n";
        return NULL;
    }

    auto secondaryValue = frameData->getValue("Secondary");
    if (!secondaryValue) {
        clog << "Secondary axis missing from two-vector frame.\n";
        return NULL;
    }

    const HashPtr& secondaryData = secondaryValue->getHash();
    if (!secondaryData) {
    ...
    ReferenceFrame::Pointer frame;
    if (primaryVector && secondaryVector) {
        frame = std::make_shared<TwoVectorFrame>(center, *primaryVector, primaryAxis, *secondaryVector, secondaryAxis);
    }

    return frame;
}

static ReferenceFramePtr CreateJ2000EclipticFrame(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
    Selection center = getFrameCenter(universe, frameData, defaultCenter);
...
static ReferenceFramePtr CreateJ2000EquatorFrame(const Universe& universe, const HashPtr& frameData, const Selection& defaultCenter) {
    Selection center = getFrameCenter(universe, frameData, defaultCenter);
...
ReferenceFramePtr CreateTopocentricFrame(const Selection& center, const Selection& target, const Selection& observer) {
    ReferenceFramePtr eqFrame = std::make_shared<BodyMeanEquatorFrame>(target, target);
 ...
 * } </pre>
 */
static ReferenceFramePtr CreateTopocentricFrame(const Universe& universe,
                                              const HashPtr& frameData,
...
static ReferenceFramePtr CreateComplexFrame(const Universe& universe,
                                          const HashPtr& frameData,
                                          const Selection& defaultCenter,
                                          const BodyPtr& defaultObserver) {
    auto value = frameData->getValue("BodyFixed");
    if (value != NULL) {
