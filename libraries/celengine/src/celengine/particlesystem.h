...
public:
    using Pointer = std::shared_ptr<ParticleEmitter>;
    ...
    //void render(double tsec, RenderContext& rc, ParticleVertex* particleBuffer, uint32_t bufferCapacity) const;
...
public:
    ...
    VectorGeneratorPtr m_positionGenerator;
    VectorGeneratorPtr m_velocityGenerator;
...
public:
    using Pointer = std::shared_ptr<ParticleSystem>;
    ParticleSystem();
    ...
    void addEmitter(const ParticleEmitter::Pointer& emitter);

public:
    std::list<ParticleEmitter::Pointer> m_emitterList;

    cmod::Mesh::VertexDescription* m_vertexDesc;
    ParticleVertex* m_vertexData;
    uint32_t m_particleCapacity;
...
public:
    BoxGenerator(const Eigen::Vector3f& center, const Eigen::Vector3f& axes) : m_center(center), m_semiAxes(axes * 0.5f) {}
...
public:
    LineGenerator(const Eigen::Vector3f& p0, const Eigen::Vector3f& p1) : m_origin(p0), m_direction(p1 - p0) {}
...
public:
    EllipsoidSurfaceGenerator(const Eigen::Vector3f& center, const Eigen::Vector3f& semiAxes) :
        m_center(center), m_semiAxes(semiAxes) {}
...
public:
    ConeGenerator(float minAngle, float maxAngle, float minLength, const float maxLength) :
        m_cosMinAngle(1.0f - std::cos(minAngle)), m_cosAngleVariance(std::cos(minAngle) - std::cos(maxAngle)),
        m_minLength(minLength), m_lengthVariance(maxLength - minLength) {}
...
public:
    GaussianDiscGenerator(float sigma) : m_sigma(sigma) {}
