...
public:
    ...
    MarkerRepresentation(Symbol symbol = MarkerRepresentation::Diamond,
        ...
        m_symbol(symbol),
        m_size(size), m_color(color), m_label(label) {}
...
class Marker {
public:
    Marker() : m_occludable{ true }, m_sizing{ ConstantSize } {}
    Marker(const Selection& obj, int priority, const MarkerRepresentation& rep, bool occludable, MarkerSizing sizing) :
        m_object(obj), m_priority(priority), m_representation(rep), m_occludable(occludable), m_sizing(sizing) {}
