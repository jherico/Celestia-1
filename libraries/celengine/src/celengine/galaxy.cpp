...
static const uint32_t GALAXY_POINTS = 3500;
...
static GalaxyTypeName GalaxyTypeNames[] = {
    { "S0", Galaxy::S0 },   { "Sa", Galaxy::Sa },   { "Sb", Galaxy::Sb },   { "Sc", Galaxy::Sc },
    { "SBa", Galaxy::SBa }, { "SBb", Galaxy::SBb }, { "SBc", Galaxy::SBc }, { "E0", Galaxy::E0 },
    { "E1", Galaxy::E1 },   { "E2", Galaxy::E2 },   { "E3", Galaxy::E3 },   { "E4", Galaxy::E4 },
    { "E5", Galaxy::E5 },   { "E6", Galaxy::E6 },   { "E7", Galaxy::E7 },   { "Irr", Galaxy::Irr },
};
...
Galaxy::Galaxy() : detail(1.0f), customTmpName(NULL), form(NULL) {
}
...
bool Galaxy::pick(const Ray3d& ray, double& distanceToPicker, double& cosAngleToBoundCenter) const {
    if (!isVisible())
        return false;
    ...
    Vector3d ellipsoidAxes(getRadius() * (form->scale.x() + RADIUS_CORRECTION), getRadius() * yscale,
                           getRadius() * (form->scale.z() + RADIUS_CORRECTION));

    Matrix3d rotation = getOrientation().cast<double>().toRotationMatrix();
    return testIntersection(Ray3d(ray.origin - getPosition(), ray.direction).transform(rotation), Ellipsoidd(ellipsoidAxes),
                            distanceToPicker, cosAngleToBoundCenter);
}

bool Galaxy::load(const HashPtr& params, const string& resPath) {
    double detail = 1.0;
...
//    uint32_t nPoints = (uint32_t) (points->size() * clamp(getDetail()));
                                   ...
                                   float pixelSize)
{
    ...
    uint32_t nRings = (uint32_t) (discSizeInPixels / 4.0f);
    uint32_t nSlices = (uint32_t) (discSizeInPixels / 4.0f);
...
uint32_t Galaxy::getRenderMask() const
{
    return Renderer::ShowGalaxies;
}


uint32_t Galaxy::getLabelMask() const
{
...
GalacticForm* buildGalacticForms(const std::string& filename) {
    ...
    for (int i = 0; i < width * height; i++) {
        ...
        if (value > 10) {
            ...
            uint32_t rr = (uint32_t)(b.position.head(3).norm() * 511);
...
void InitializeForms() {
    ...
    for (uint32_t eform = 0; eform <= 7; ++eform) {
        ...
        if (*ellipticalForms) {
            uint32_t nPoints = (uint32_t)(ellipticalForms[eform]->blobs->size());
            for (uint32_t i = 0; i < nPoints; ++i) {
                (*ellipticalForms[eform]->blobs)[i].colorIndex =
                    (uint32_t)ceil(0.76f * (*ellipticalForms[eform]->blobs)[i].colorIndex);
            }
        }
    }
    //Irregular Galaxies
    uint32_t galaxySize = GALAXY_POINTS, ip = 0;
    Blob b;
    Point3f p;
