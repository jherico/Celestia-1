...
LODSphereMesh::LODSphereMesh() : vertices(NULL), vertexBuffersInitialized(false), useVertexBuffers(false) {
...
static Point3f spherePoint(int theta, int phi) {
    return Point3f(cosPhi[phi] * cosTheta[theta], sinPhi[phi], cosPhi[phi] * sinTheta[theta]);
}
...
//                vertexBuffers[i] = (uint32_t) vbname;
