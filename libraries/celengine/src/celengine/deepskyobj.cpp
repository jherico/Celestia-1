...
DeepSkyObject::DeepSkyObject() :
    catalogNumber(InvalidCatalogNumber), position(0, 0, 0), orientation(Quaternionf::Identity()), radius(1),
    absMag(DSO_DEFAULT_ABS_MAGNITUDE), infoURL(NULL), visible(true), clickable(true) {
}
...
bool DeepSkyObject::pick(const Ray3d& ray, double& distanceToPicker, double& cosAngleToBoundCenter) const {
    if (isVisible())
...
bool DeepSkyObject::load(const HashPtr& params, const string& resPath) {
    // Get position
...
#ifdef CELVEC
    Quatf q(1);
    q.setAxisAngle(Vec3f((float)axis.x, (float)axis.y, (float)axis.z), (float)degToRad(angle));
