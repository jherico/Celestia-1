...
public:
    AppProgressNotifier(CelestiaAppWindow* _appWin) : appWin(_appWin) {}

    void update(const string& s) { appWin->loadingProgressUpdate(QString(s.c_str())); }
...
public:
    AppAlerter(QWidget* _parent) : parent(_parent) {}

    ~AppAlerter() {}

    void fatalError(const string& msg) { QMessageBox::critical(parent, "Celestia", QString(msg.c_str())); }
...
CelestiaAppWindow::CelestiaAppWindow(QWidget* parent) :
    ...
    celestialBrowser(NULL), m_appCore(NULL), actions(NULL), infoPanel(NULL), eventFinder(NULL), alerter(NULL),
    m_preferencesDialog(NULL), m_bookmarkManager(NULL), m_bookmarkToolBar(NULL) {
...
void CelestiaAppWindow::init(const QString& qConfigFileName, const QStringList& qExtrasDirectories) {
    ...
    for (QStringList::const_iterator iter = qExtrasDirectories.begin(); iter != qExtrasDirectories.end(); iter++) {
    ...
    m_appCore->initSimulation(&configFileName, &extrasDirectories, progress);
    delete progress;
    ...
    if (glewErr != GLEW_OK) {
        QMessageBox::
            critical(0, "Celestia",
                     QString(
                         _("Celestia was unable to initialize OpenGL extensions (error %1). Graphics quality will be reduced."))
                         .arg(glewErr));
    }
    ...
    connect(celestialBrowser, SIGNAL(selectionContextMenuRequested(const QPoint&, Selection&)), this,
    ...
    connect(deepSkyBrowser, SIGNAL(selectionContextMenuRequested(const QPoint&, Selection&)), this,
            SLOT(slotShowSelectionContextMenu(const QPoint&, Selection&)));

    SolarSystemBrowser* solarSystemBrowser = new SolarSystemBrowser(m_appCore, NULL);
    solarSystemBrowser->setObjectName("ssys-browser");
    connect(solarSystemBrowser, SIGNAL(selectionContextMenuRequested(const QPoint&, Selection&)), this,
            SLOT(slotShowSelectionContextMenu(const QPoint&, Selection&)));
    ...
    eventFinder->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    ...
    connect(m_bookmarkManager, SIGNAL(bookmarkTriggered(const QString&)), this, SLOT(slotBookmarkTriggered(const QString&)));
...
bool CelestiaAppWindow::loadBookmarks() {
    ...
    if (!QFile::exists(bookmarksFilePath)) {
        QMessageBox::warning(this, _("No Bookmarks File"),
                             QString(_("Bookmarks file %1 does not exist.")).arg(bookmarksFilePath));
    } else {
...
void CelestiaAppWindow::slotShowSelectionContextMenu(const QPoint& pos, Selection& sel) {
    SelectionPopup* menu = new SelectionPopup(sel, m_appCore, this);
    connect(menu, SIGNAL(selectionInfoRequested(Selection&)), this, SLOT(slotShowObjectInfo(Selection&)));
...
void CelestiaAppWindow::slotGrabImage() {
    ...
    QString saveAsName = QFileDialog::getSaveFileName(this, _("Save Image"), dir, _("Images (*.png *.jpg)"));
...
// TODO: Add Mac support
    ...
    int videoSizes[8][2] = { { 160, 120 }, { 320, 240 },  { 640, 480 },  { 720, 480 },
                             { 720, 576 }, { 1024, 768 }, { 1280, 720 }, { 1920, 1080 } };

    float videoFrameRates[5] = { 15.0f, 24.0f, 25.0f, 29.97f, 30.0f };

#ifdef _WIN32
    QString saveAsName = QFileDialog::getSaveFileName(this, _("Capture Video"), dir, _("Video (*.avi)"));
#else
    QString saveAsName = QFileDialog::getSaveFileName(this, _("Capture Video"), dir, _("Video (*.ogv)"));
#endif
    if (!saveAsName.isEmpty()) {
        ...
        for (unsigned int i = 0; i < sizeof(videoSizes) / sizeof(videoSizes[0]); i++) {
            resolutionCombo->addItem(QString("%1 x %2").arg(videoSizes[i][0]).arg(videoSizes[i][1]),
                                     QSize(videoSizes[i][0], videoSizes[i][1]));
        }
        ...
        QDialogButtonBox* buttons =
            new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, Qt::Horizontal, &videoInfoDialog);
...
#endif
            bool ok = movieCapture->start(saveAsName.toLatin1().data(), videoSize.width(), videoSize.height(), frameRate);
            if (ok)
...
void CelestiaAppWindow::slotShowObjectInfo(Selection& sel) {
    infoPanel->buildInfoPage(sel, m_appCore->getSimulation()->getUniverse(), m_appCore->getSimulation()->getTime());
...
void CelestiaAppWindow::slotOpenScriptDialog() {
    ...
    QString scriptFileName = QFileDialog::getOpenFileName(this, _("Open Script"), dir, _("Celestia Scripts (*.celx *.cel)"));
...
void CelestiaAppWindow::slotShowTimeDialog() {
    SetTimeDialog* timeDialog = new SetTimeDialog(m_appCore->getSimulation()->getTime(), this, m_appCore);
...
void CelestiaAppWindow::slotAddBookmark() {
    ...
    if (sel.body() != NULL) {
        ...
        defaultTitle =
            QString::fromUtf8(ReplaceGreekLetterAbbr(universe->getStarCatalog()->getStarName(*sel.star(), true)).c_str());
    } else if (sel.deepsky() != NULL) {
        Universe* universe = m_appCore->getSimulation()->getUniverse();
        defaultTitle =
            QString::fromUtf8(ReplaceGreekLetterAbbr(universe->getDSOCatalog()->getDSOName(sel.deepsky(), true)).c_str());
...
void CelestiaAppWindow::slotManual() {
    ...
    QDesktopServices::openUrl(QUrl(QUrl::fromLocalFile(QDir::toNativeSeparators(QApplication::applicationDirPath()) +
                                                       QDir::toNativeSeparators(QDir::separator()) + "help" +
                                                       QDir::toNativeSeparators(QDir::separator()) + MANUAL_FILE)));
...
void CelestiaAppWindow::slotShowAbout() {
    static const char* aboutText = gettext_noop(
        "<html>"
...
void CelestiaAppWindow::slotShowGLInfo() {
    ...
    if (extensions != NULL) {
        ...
        foreach (QString s, extList) { out << s << "<br>\n"; }
    }
...
void CelestiaAppWindow::createMenus() {
    ...
    QAction* grabImageAction = new QAction(QIcon(":/icons/grab-image.png"), _("&Grab image"), this);
    ...
    QAction* captureVideoAction = new QAction(QIcon(":/icons/capture-video.png"), _("Capture &video"), this);
    ...
    QAction* prefAct = new QAction(QIcon(":/icons/preferences.png"), _("&Preferences..."), this);
    ...
    QAction* splitViewVertAction = new QAction(QIcon(":/icons/split-vert.png"), _("Split view vertically"), this);
    ...
    QAction* splitViewHorizAction = new QAction(QIcon(":/icons/split-horiz.png"), _("Split view horizontally"), this);
    ...
    QAction* cycleViewAction = new QAction(QIcon(":/icons/split-cycle.png"), _("Cycle views"), this);
    ...
    QAction* singleViewAction = new QAction(QIcon(":/icons/split-single.png"), _("Single view"), this);
    ...
    QAction* deleteViewAction = new QAction(QIcon(":/icons/split-delete.png"), _("Delete view"), this);
...
void CelestiaAppWindow::populateBookmarkMenu() {
    ...
    QAction* organizeBookmarksAction =
        new QAction(QIcon(":/icons/application-bookmark.png"), _("Organize Bookmarks..."), bookmarkMenu);
...
QMenu* CelestiaAppWindow::buildScriptsMenu() {
    ...
    for (vector<ScriptMenuItem>::const_iterator iter = scripts->begin(); iter != scripts->end(); iter++) {
